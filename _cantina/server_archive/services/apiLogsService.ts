...
export function addLog(log: Omit<APILog, "id" | "timestamp">): APILog {
  ...
  console.log(
    `[${newLog.level.toUpperCase()}] [${newLog.app}] ${newLog.message}`
  );
...
export function cleanOldLogs(days: number = 7): number {
  ...
  console.log(
    `[API Logs Service] Rimossi ${removed} log più vecchi di ${days} giorni`
  );
...
export function initDemoLogs() {
  ...
  console.log(
    `[API Logs Service] Inizializzati ${demoLogs.length} log di demo`
  );
}
