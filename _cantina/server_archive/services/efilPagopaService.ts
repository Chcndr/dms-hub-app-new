...
function getEfilConfig(): EfilConfig {
  const baseUrl =
    process.env.EFIL_BASE_URL || "https://test.plugnpay.efil.it/plugnpay";

  return {
    baseUrl,
    wspaymentWsdl:
      process.env.EFIL_WSPAYMENT_WSDL || `${baseUrl}/WSPayment?wsdl`,
    wsfeedWsdl: process.env.EFIL_WSFEED_WSDL || `${baseUrl}/WSFeed?wsdl`,
    wsdeliverWsdl:
      process.env.EFIL_WSDELIVER_WSDL || `${baseUrl}/WSDeliver?wsdl`,
    wsgeneratorpdfWsdl:
      process.env.EFIL_WSGENERATORPDF_WSDL || `${baseUrl}/WSGeneratorPdf?wsdl`,
    wspaymentnotifyWsdl:
      process.env.EFIL_WSPAYMENTNOTIFY_WSDL ||
      `${baseUrl}/WSPaymentNotify?wsdl`,
    ...
    returnUrl:
      process.env.DMS_PAGOPA_RETURN_URL || "https://miohub.app/payments/return",
    callbackUrl:
      process.env.DMS_PAGOPA_CALLBACK_URL ||
      "https://miohub.app/payments/callback",
  };
}
 ...
 */
export async function creaPosizione(
 ...
 */
export async function statoPerIuv(
  req: GetByIuvRequest
): Promise<GetByIuvResponse> {
  const config = getEfilConfig();
...
export async function generaPdf(
  req: GeneratorPdfRequest
): Promise<GeneratorPdfResponse> {
  const config = getEfilConfig();
