  ...
  try {
    const response = await axios.get(url, {
      params: {
        ...
        select:
          "codice,codice_linea,denominazione,ubicazione,comune,geopoint,quartiere",
      },
      timeout: 15000, // 15s timeout for external TPER API
    });

    console.log(
      `[TPER Service] Risposta API TPER (Status: ${response.status}, Body: ${JSON.stringify(response.data).substring(0, 200)}...)`
    );
    ...
    console.error(
      "[TPER Service] Errore nel recupero delle fermate:",
      error.message,
      error.stack
    );
 ...
 */
export async function getTPERBusTimes(
  stopCode: number,
  lineNumber: string
): Promise<TPERBusTime | null> {
  try {
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <soap:Body>
    ...
    const busResult =
      result["soap:Envelope"]["soap:Body"][0]["QueryHellobusResponse"][0][
        "QueryHellobusResult"
      ][0];
    ...
    console.error(
      `[TPER Service] Errore nel recupero orari per fermata ${stopCode}, linea ${lineNumber}:`,
      error.message
    );
    return null;
  }
...
export async function syncTPERData() {
  ...
  try {
    ...
    console.log(
      `[TPER Service] Sincronizzati ${mobilityData.length} dati mobilità`
    );
    return mobilityData;
  } catch (error: any) {
    console.error(
      "[TPER Service] Errore durante la sincronizzazione:",
      error.message
    );
    throw error;
  }
...
export async function updateTPERRealtimeData() {
  ...
  try {
    ...
    const stops = await db
      .select()
      .from(mobilityData)
      .where(eq(mobilityData.type, "bus"));
    console.log(
      `[TPER Service] Recuperate ${stops.length} fermate dal database`
    );
    ...
    console.error(
      "[TPER Service] Errore durante l'aggiornamento real-time:",
      error.message
    );
    throw error;
  }
