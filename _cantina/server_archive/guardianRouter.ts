...
clearAllLogs();
console.log(
  "[Guardian Router] Log di demo rimossi - sistema pronto per log reali"
);
    ...
    .input(
      z
        .object({
          ...
          type: z
            .enum(["API_CALL", "DATABASE", "INTEGRATION", "SECURITY", "ERROR"])
            .optional(),
  ...
  testEndpoint: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const apiInfo = getAPIById(
          endpoint.replace("/api/", "").replace(/\//g, ".")
        );
