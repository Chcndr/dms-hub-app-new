...
export default function SuapDashboard({
  ...
  useEffect(() => {
    async function loadPratiche() {
      try {
        const data = await getSuapPratiche(
  ...
  const handleSciaSubmit = async (data: any) => {
    try {
      ...
      const praticaData = {
        // Dati base
        tipo_pratica:
          `SCIA ${data.tipo_segnalazione || data.motivazione_scia || "Subingresso"}`.toUpperCase(),
        richiedente_nome:
          data.ragione_sociale_sub ||
          `${data.nome_sub || ""} ${data.cognome_sub || ""}`.trim() ||
          "Non specificato",
      ...
      const newPratica = await createSuapPratica(
        "00000000-0000-0000-0000-000000000001",
        praticaData
      );
      ...
      try {
        const updatedPratiche = await getSuapPratiche(
          "00000000-0000-0000-0000-000000000001"
        );
        setPratiche(
          updatedPratiche
      ...
      try {
        const newStats = await getSuapStats(
          "00000000-0000-0000-0000-000000000001"
        );
        setStats(newStats);
      } catch (e) {
  ...
  if (loading) {
    return (
      <div className="p-8 text-[#e8fbff]">Caricamento dashboard SUAP...</div>
    );
  }

  return (
    <div
      className={`space-y-8 ${embedded ? "" : "p-8 bg-[#020817] min-h-screen"}`}
    >
      {/* Header */}
      {!embedded && (
          ...
          <div className="w-full max-w-4xl my-8">
            <ConcessioneForm
              ...
              onSubmit={async data => {
                ...
                toast.success("Concessione Rilasciata", {
                  description: `N. ${data.numero_concessione}`,
                });
              }}
            />
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-[#0a1628] border-[#1e293b]">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-[#e8fbff]/80">
              Totale Pratiche
            </CardTitle>
            ...
            <div className="space-y-4">
              {loadingPendenti ? (
                ...
                pratichePendenti.map(pratica => (
                  <div
                    key={pratica.id}
                    className="flex items-center p-3 rounded-lg bg-orange-500/5 border border-orange-500/20 hover:bg-orange-500/10 cursor-pointer transition-colors"
                  >
                    ...
                    <div className="ml-auto text-right">
                      <p className="text-xs text-[#e8fbff]/60">
                        {formatRelativeTime(pratica.data)}
                      </p>
                      <Button
                        size="sm"
                        variant="ghost"
                        className="text-orange-400 hover:bg-orange-400/10 mt-1"
                      >
                        Verifica
                      </Button>
                    </div>
            ...
            <div className="space-y-3">
              {loadingNuove ? (
                ...
                nuoveDomande.map(domanda => (
                  <div
                    key={domanda.id}
                    className="flex items-center p-2 rounded-lg hover:bg-blue-500/5 cursor-pointer transition-colors border border-transparent hover:border-blue-500/20"
                  >
                    ...
                    <span className="text-xs text-[#e8fbff]/40">
                      {formatRelativeTime(domanda.data)}
                    </span>
                  </div>
                ))
              )}
