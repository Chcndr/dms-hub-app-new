...
## 🔥 SEZIONE HEATMAP - ZONE DI CALORE

### 11.1 Cos'è una Heatmap

Una **heatmap** (mappa di calore) è una visualizzazione che mostra la **densità** delle segnalazioni in un'area geografica:

- **Zone rosse/arancioni** = Alta concentrazione di segnalazioni
- **Zone gialle** = Media concentrazione
- **Zone verdi/blu** = Bassa concentrazione
- **Zone trasparenti** = Nessuna segnalazione

### 11.2 Libreria Utilizzata

Utilizzeremo **react-leaflet-heatmap-layer-v3** che è compatibile con React 18+ e Leaflet 1.9+.

```bash
# Installazione
pnpm add react-leaflet-heatmap-layer-v3
```

### 11.3 Schema Architettura Heatmap
...
│  │  │  │                    LAYER STACK                               │   │ │  │
│  │  │  │                                                              │   │ │  │
│  │  │  │  Layer 1: TileLayer (mappa base)                            │   │ │  │
│  │  │  │           ↓                                                  │   │ │  │
│  │  │  │  Layer 2: 🔥 HeatmapLayer (zone calore)                     │   │ │  │
│  │  │  │           ↓                                                  │   │ │  │
│  │  │  │  Layer 3: Marker Mercati/HUB/Negozi (esistenti)             │   │ │  │
│  │  │  │           ↓                                                  │   │ │  │
│  │  │  │  Layer 4: 🆕 Marker Civici (puntini colorati)               │   │ │  │
│  │  │  │                                                              │   │ │  │
│  │  │  └─────────────────────────────────────────────────────────────┘   │ │  │
│  │  │                                                                     │ │  │
│  │  └────────────────────────────────────────────────────────────────────┘ │  │
...
│  │  │                    CONTROLLI LAYER                                  │ │  │
│  │  │                                                                     │ │  │
│  │  │  [✓] Mostra Heatmap    [✓] Mostra Marker Civici                    │ │  │
│  │  │                                                                     │ │  │
│  │  └────────────────────────────────────────────────────────────────────┘ │  │
│  │                                                                          │  │
│  └─────────────────────────────────────────────────────────────────────────┘  │
...
### 11.4 Configurazione Heatmap

```typescript
// Configurazione heatmap per segnalazioni civiche
const heatmapConfig = {
  // Raggio di influenza di ogni punto (in pixel)
  radius: 25,
  ...
  // Gradiente colori (da freddo a caldo)
  gradient: {
...
### 11.5 Formato Dati Heatmap

La heatmap richiede un array di punti nel formato `[lat, lng, intensity]`:

```typescript
// Trasformazione dati civicReports → heatmapPoints
const heatmapPoints = civicReports
  .filter(r => r.lat && r.lng) // Solo con coordinate valide
  .map(report => {
    const lat = parseFloat(report.lat);
    const lng = parseFloat(report.lng);
    
    // Intensità basata su priorità
    let intensity = 1;
    if (report.priority === 'HIGH') intensity = 2;
    if (report.priority === 'URGENT') intensity = 3;
    
    // Boost per segnalazioni pending (più urgenti)
...
### 11.6 Implementazione Componente

```tsx
import { HeatmapLayer } from 'react-leaflet-heatmap-layer-v3';

// Dentro HubMarketMapComponent
...
### 11.7 Toggle Heatmap/Marker

L'utente potrà scegliere di visualizzare:
- Solo Heatmap
- Solo Marker
- Entrambi (default)
...
interface HubMarketMapComponentProps {
  // ... altre props ...
  civicReports?: CivicReport[];
  showCivicHeatmap?: boolean;  // Default: true
  showCivicMarkers?: boolean;  // Default: true
}
```
...
### 12.1 Nuova Struttura civic_reports

```sql
CREATE TABLE civic_reports (
  id              INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
  user_id         INTEGER REFERENCES users(id),
  comune_id       INTEGER,                          -- 🆕 Per filtro impersonificazione
  ...
  created_at      TIMESTAMP DEFAULT NOW(),
  updated_at      TIMESTAMP DEFAULT NOW()           -- 🆕 Data aggiornamento
);

-- Indici per performance
CREATE INDEX idx_civic_reports_comune ON civic_reports(comune_id);
CREATE INDEX idx_civic_reports_status ON civic_reports(status);
CREATE INDEX idx_civic_reports_priority ON civic_reports(priority);
```
...
|-------|------|-------------|
| `comune_id` | INTEGER | FK per filtro impersonificazione |
| `priority` | VARCHAR(20) | LOW, NORMAL, HIGH, URGENT |
| `address` | TEXT | Indirizzo da reverse geocoding |
| `updated_at` | TIMESTAMP | Data ultimo aggiornamento |

---

## 🔄 FLUSSO COMPLETO CON HEATMAP

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                     FLUSSO DATI MARKER + HEATMAP                              │
└──────────────────────────────────────────────────────────────────────────────┘

1. QUERY API (con filtro comune_id)
   ┌─────────────────┐
   │  DashboardPA    │
   │                 │
   │  GET /api/      │
...
3. TRASFORMAZIONE
   ┌─────────────────────────────────────────────────────────────┐
   │                                                              │
   │  civicReports ──┬──▶ heatmapPoints [[lat, lng, intensity]]  │
   │                 │                                            │
   │                 └──▶ markerData (per rendering marker)       │
   │                                                              │
   └────────────────────────────────────────────────────────────┘
            │
            ▼
4. RENDERING MAPPA
   ...
   │  │          ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░ ││
   │  ├─────────────────────────────────────────────────────────┤│
   │  │ Layer 3: Marker Mercati/HUB (esistenti)                 ││
   │  │          🔴M  🔵H  🟢●                                   ││
   │  ├─────────────────────────────────────────────────────────┤│
   │  │ Layer 4: 🆕 Marker Civici                               ││
   │  │          🟠● 🟡● 🟢● 🔴● 🟣●                              ││
   │  │          (lampeggianti se URGENT)                       ││
   │  └─────────────────────────────────────────────────────────┘│
   │                                                              │
   └─────────────────────────────────────────────────────────────┘
```
...
- [x] Aggiunto campo `comune_id`
- [x] Aggiunto campo `priority`
- [x] Aggiunto campo `address`
- [x] Aggiunto campo `updated_at`

### Fase 2: Marker Civici (DA FARE)
- [ ] Installare dipendenze heatmap
- [ ] Definire interfaccia CivicReport con campi opzionali
- [ ] Aggiungere costante CIVIC_MARKER_COLORS
- [ ] Aggiungere prop civicReports a HubMarketMapComponent
- [ ] Implementare rendering marker con icone colorate
- [ ] Aggiungere animazione pulse per URGENT

### Fase 3: Heatmap (DA FARE)
- [ ] Aggiungere prop showCivicHeatmap
- [ ] Trasformare civicReports in heatmapPoints
- [ ] Aggiungere HeatmapLayer con gradiente colori
- [ ] Testare performance con molti punti

### Fase 4: Integrazione (DA FARE)
- [ ] Aggiornare GestioneHubMapWrapper
- [ ] Aggiornare DashboardPA con passaggio dati
- [ ] Aggiungere toggle heatmap/marker (opzionale)
- [ ] Testare filtro per comune_id

### Fase 5: Deploy (DA FARE)
- [ ] Build locale senza errori
- [ ] Push su GitHub
- [ ] Verifica deploy Vercel
...
> **Versione:** 1.1.0 (aggiunta sezione Heatmap)
