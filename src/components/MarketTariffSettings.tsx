...
export function MarketTariffSettings({ marketId, marketName }: MarketTariffSettingsProps) {
  ...
  const fetchTariffs = async () => {
    try {
      ...
      if (data.success) {
        ...
        const currentYearTariff = data.data.find((t: Tariff) => t.year === new Date().getFullYear());
  ...
  return (
    <Card className="mt-4">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-medium">Configurazione Tariffe - {marketName}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="mt-4">
            <h4 className="text-sm font-medium mb-2 text-gray-500">Storico Tariffe</h4>
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {tariffs.length === 0 ? (
                <p className="text-xs text-gray-400">Nessuna tariffa configurata</p>
              ) : (
                tariffs.map((t) => (
                  <div key={t.id} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded">
                    ...
                    <div className="text-right">
                      <div className="font-bold">€ {Number(t.tariff_per_sqm).toFixed(2)} /mq</div>
                      <div className="text-xs text-gray-500">{t.market_days_per_year} giorni</div>
                    </div>
                  </div>
                ))
              )}
