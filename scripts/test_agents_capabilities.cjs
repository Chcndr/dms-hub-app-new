...
async function testToolsDefinition() {
  ...
  const toolsPath = path.join(
    __dirname,
    "../.mio-agents/tools_definition.json"
  );
...
async function testAPIReference() {
  ...
  const apiRefPath = path.join(
    __dirname,
    "../.mio-agents/api_reference_for_agents.md"
  );
...
async function testDatabaseConnection() {
  ...
  try {
    ...
    const marketsCount = await pool.query(
      "SELECT count(*) as total FROM markets"
    );
    const stallsCount = await pool.query(
      "SELECT count(*) as total FROM stalls"
    );
...
async function runTests() {
  ...
  console.log("");
  console.log(
    `${colors.blue}Result: ${passed}/${total} tests passed (${percentage}%)${colors.reset}`
  );
