...
function parseTRPCRouter(filePath, fileName) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const procedureMatch = line.match(
      /^\s+(\w+):\s+(publicProcedure|protectedProcedure)/
    );
    if (procedureMatch) {
      ...
      endpoints.push({
        id: currentSubrouter
          ? `${currentSubrouter}.${procedureName}`
          : procedureName,
        method,
        path,
        category:
  ...
  for (const newEp of newEndpoints) {
    ...
    if (existing) {
      ...
      mergedEndpoints.push({
        ...
        implementation_note:
          newEp.implementation_note || existing.implementation_note,
...
function main() {
  ...
  log("\n🔀 Merge con index.json esistente...");
  const finalData = mergeWithExistingIndex(
    allEndpoints,
    CONFIG.mioHubIndexPath
  );
  ...
  log(
    `Endpoint in index.json: ${finalData.services[0]?.endpoints.length || 0}`
  );
