...
export function useSystemStatus(pollInterval: number = 30000): SystemStatusResult {
  ...
  const checkBackendAPI = async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch('https://mihub.157-90-29-66.nip.io/api/system/health', {
  ...
  const checkPM2Status = async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch('https://mihub.157-90-29-66.nip.io/api/system/pm2-status', {
        signal: controller.signal,
        method: 'GET',
      });
