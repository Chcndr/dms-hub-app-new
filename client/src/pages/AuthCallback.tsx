  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      if (errorParam) {
        setStatus("error");
        setError(
          errorDescription || errorParam || "Errore durante l'autenticazione"
        );
        return;
      }
      ...
      try {
        ...
        setStatus("error");
        setError(
          err instanceof Error ? err.message : "Errore durante l'autenticazione"
        );
      }
    };
      ...
      <div className="max-w-md w-full p-8 bg-slate-800/50 backdrop-blur border border-slate-700 rounded-2xl text-center">
        ...
        {status === "success" && (
          <>
            ...
            <p className="mt-2 text-gray-400">Reindirizzamento in corso...</p>
          </>
        )}
