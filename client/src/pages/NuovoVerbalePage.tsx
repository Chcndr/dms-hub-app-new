...
export default function NuovoVerbalePage() {
  ...
  const [comuni, setComuni] = useState<
    { id: number; nome: string; provincia: string }[]
  >([]);
  ...
  const [agentQualifica, setAgentQualifica] = useState(
  ...
  const [transgressorDocReleasedBy, setTransgressorDocReleasedBy] =
    useState("");
  ...
  const [selectedInfrazione, setSelectedInfrazione] =
    useState<Infrazione | null>(null);
  ...
  useEffect(() => {
    if (isImpersonating && impersonatedComuneId && comuni.length > 0) {
      ...
      if (comuneExists) {
        ...
        console.warn(
          "[Verbale] Impersonificazione: selezionato comune",
          impersonatedComuneId
        );
      }
    }
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const urlComuneId = impState.comuneId
        ? parseInt(impState.comuneId)
        : null;

      console.warn(
        "[Verbale] Impersonation params - impersonate:",
        urlImpersonate,
        "comune_id:",
        urlComuneId,
        "(from:",
        impState.comuneId ? "storage/url" : "none",
        ")"
      );
      ...
      if (comuniData.success) {
        ...
        if (urlImpersonate && urlComuneId) {
          // Verifica che il comune esista nella lista
          const comuneExists = comuniData.data?.some(
            (c: any) => c.id === urlComuneId
          );
          if (comuneExists) {
            setSelectedComuneId(urlComuneId);
            console.warn(
              "[Verbale] Usando comune da impersonificazione:",
              urlComuneId
            );
          } else {
            console.warn(
              "[Verbale] Comune impersonificato non trovato:",
              urlComuneId
            );
  ...
  const handleSelectImpresa = async (impresa: Impresa) => {
    ...
    setTransgressorCF(
      impresa.rappresentante_legale_cf || impresa.codice_fiscale || ""
    );

    // Data e luogo di nascita
    if (impresa.rappresentante_legale_data_nascita) {
      setTransgressorBirthDate(
        impresa.rappresentante_legale_data_nascita.split("T")[0]
      );
    }
    setTransgressorBirthPlace(
      impresa.rappresentante_legale_luogo_nascita || ""
    );
  ...
  // Filter infrazioni
  const filteredInfrazioni = infrazioni.filter(
    inf =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const corpoPM = comuneNome
        ? `Polizia Municipale di ${comuneNome.replace("Comune di ", "")}`
        : "Polizia Municipale";
      ...
      if (result.success) {
        ...
        try {
          await authenticatedFetch(`${MIHUB_API}/notifiche/send`, {
            ...
            body: JSON.stringify({
              ...
              messaggio:
                `È stato emesso il verbale ${result.data.verbale_code} per infrazione ${selectedInfrazione?.code || ""}. Importo: €${parseFloat(amount || "0").toFixed(2)}. ${violationDescription || ""}`.trim(),
        ...
        alert(
          `✅ Verbale ${result.data.verbale_code} emesso con successo!${notificaInviata ? "\n\nNotifica inviata." : "\n\n⚠️ Notifica non inviata (errore di rete)."}`
        );
      ...
      <div className="min-h-screen bg-[#0b1220] flex items-center justify-center">
        <div className="text-center">
          ...
          <Button
            ...
            className="bg-[#f59e0b] text-black"
          >
            Riprova
          </Button>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold">
                    Nuovo Verbale di Contestazione
                  </h1>
                  <p className="text-sm text-[#e8fbff]/60">
                    Art. 14 L. 689/81 - Violazione Amministrativa
                  </p>
                </div>
              </div>
            </div>
          ...
          <Card className="bg-[#1a2332] border-[#3b82f6]/30">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-[#e8fbff]/60">
                Dati dell'Ente e del Corpo di Polizia Municipale
              </CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={
                    comuni.find(c => c.id === selectedComuneId)?.provincia || ""
                  }
                  disabled
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Input
                  value={
                    selectedComuneId
                      ? `Polizia Municipale di ${comuni.find(c => c.id === selectedComuneId)?.nome || ""}`
                      : ""
                  }
                  disabled
                  className="bg-[#0b1220]/50 border-[#3b82f6]/30 text-[#e8fbff]"
          ...
          <Card className="bg-[#1a2332] border-[#f59e0b]/30">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-[#e8fbff]">
                  2. Agente Accertatore
                </CardTitle>
              </div>
              <CardDescription className="text-[#e8fbff]/60">
                Dati dell'agente che redige il verbale
              </CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="Agente di Polizia Municipale">
                    Agente di Polizia Municipale
                  </option>
                  <option value="Ispettore di Polizia Municipale">
                    Ispettore di Polizia Municipale
                  </option>
                  <option value="Commissario di Polizia Municipale">
                    Commissario di Polizia Municipale
                  </option>
                  <option value="Comandante di Polizia Municipale">
          ...
          <Card className="bg-[#1a2332] border-[#10b981]/30">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-[#e8fbff]">
                  3. Luogo e Data della Violazione
                </CardTitle>
              </div>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {selectedImpresa && (
                <div className="mt-2 p-2 bg-[#ef4444]/10 rounded-lg flex items-center justify-between">
                  <span className="text-sm text-[#ef4444]">
                    Impresa selezionata:{" "}
                    <strong>{selectedImpresa.denominazione}</strong> - P.IVA:{" "}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setTransgressorCF(e.target.value.toUpperCase())
                    }
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-[#e8fbff]">
                    5. Proprietario/Obbligato in solido
                  </CardTitle>
                </div>
              ...
              {selectedInfrazione ? (
                <div className="p-4 bg-[#ef4444]/10 rounded-lg border border-[#ef4444]/30">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <h4 className="text-lg font-semibold text-[#e8fbff] mt-2">
                        {selectedInfrazione.code}
                      </h4>
                      <p className="text-[#e8fbff]/80">
                        {selectedInfrazione.description}
                      </p>
                    </div>
                    <Button
                      type="button"
                  ...
                  <div className="mt-3 flex items-center gap-4 text-sm">
                    <span className="text-[#e8fbff]/60">
                      Importo: Min €{selectedInfrazione.min_amount} - Max €
                      {selectedInfrazione.max_amount}
                    </span>
                    <span className="text-[#ef4444] font-medium">
                      Default: €{selectedInfrazione.default_amount}
                    </span>
                  </div>
                </div>
              ) : (
                <div className="p-8 border-2 border-dashed border-[#ef4444]/30 rounded-lg text-center">
                  ...
                  <p className="text-[#e8fbff]/60">
                    Nessuna infrazione selezionata
                  </p>
                  <p className="text-sm text-[#e8fbff]/40 mt-1">
                    Clicca "Seleziona Infrazione" per scegliere dal catalogo
                  </p>
                </div>
              )}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-[#e8fbff]/70">
                    Contestazione differita (notifica successiva)
                  </span>
                </label>
              </div>
            </CardContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    value={
                      amount
                        ? `€ ${(parseFloat(amount) * 0.7).toFixed(2)}`
                        : "€ 0.00"
                    }
                    disabled
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    value={
                      violationDate
                        ? new Date(
                            new Date(violationDate).getTime() +
                              60 * 24 * 60 * 60 * 1000
          ...
          <div className="bg-[#1a2332] border border-[#ef4444]/30 rounded-lg w-full max-w-4xl max-h-[80vh] overflow-hidden">
            <div className="p-4 border-b border-[#ef4444]/30 flex items-center justify-between">
              <h3 className="text-lg font-bold text-[#e8fbff]">
                Seleziona Tipo Infrazione
              </h3>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setShowInfrazioneModal(false)}
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge className={getCategoryColor(inf.category)}>
                          {inf.category}
                        </Badge>
                      ...
                      {inf.testo_legge_completo && (
                        <p className="text-sm text-[#e8fbff]/50 mt-1 line-clamp-2">
                          {inf.testo_legge_completo}
                        </p>
                      )}
                    </div>
                    <div className="text-right ml-4">
                      ...
                      <p className="text-xs text-[#e8fbff]/40">
                        Min €{inf.min_amount} - Max €{inf.max_amount}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-[#1a2332] border border-[#3b82f6]/30 rounded-lg w-full max-w-3xl max-h-[80vh] overflow-hidden">
            <div className="p-4 border-b border-[#3b82f6]/30 flex items-center justify-between">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setShowImpresaModal(false)}
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
            ...
            <div className="overflow-y-auto max-h-[50vh] p-4 space-y-2">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-[#e8fbff]/60">
                        P.IVA: {imp.partita_iva}{" "}
                        {imp.codice_fiscale && `| C.F.: ${imp.codice_fiscale}`}
                      </p>
                      {imp.indirizzo && (
                        <p className="text-xs text-[#e8fbff]/40 mt-1">
                          {imp.indirizzo}, {imp.citta}
                        </p>
                      )}
                    </div>
              ...
              {filteredImprese.length > 50 && (
                <div className="text-center py-4 text-[#e8fbff]/40 text-sm">
                  Mostrati i primi 50 risultati. Affina la ricerca per vedere
                  altri risultati.
                </div>
              )}
            </div>
