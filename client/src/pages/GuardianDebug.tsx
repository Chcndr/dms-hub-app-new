...
export default function GuardianDebug() {
  ...
  const unknownAgents = logs.filter(log =>
    log.reason?.includes("Unknown agent")
  );
  const unknownEndpoints = logs.filter(log =>
    log.reason?.includes("Unknown endpoint")
  );
  const noPermissions = logs.filter(log =>
    log.reason?.includes("No permission")
  );
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Solo errori e accessi negati con suggerimenti risoluzione
            </p>
          </div>
        </div>
        ...
        {filteredLogs.length === 0 ? (
          <div className="bg-green-50 border border-green-200 rounded-lg p-8 text-center">
            ...
            <p className="text-green-700">
              Tutti gli accessi API sono stati autorizzati correttamente.
            </p>
          </div>
        ) : (
          filteredLogs.map((log, index) => (
            <div
              key={index}
              className="bg-white rounded-lg shadow-sm border-l-4 border-red-500 p-6"
            >
                ...
                <div className="flex-1">
                  ...
                  {log.reason && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                      <div className="text-xs text-red-600 uppercase mb-1">
                        Motivo
                      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          ...
          <br />
          <strong>File:</strong> logs/api-guardian.log (filtrato per
          status=denied/error)
          <br />
          <strong>Nota:</strong> Quando il backend Guardian sarà deployato, i
          log verranno aggiornati in tempo reale
        </p>
      </div>
    </div>
  );
