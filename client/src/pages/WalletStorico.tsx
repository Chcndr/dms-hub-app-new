...
export default function WalletStorico() {
  ...
  const totalTCC = walletStats
    ? walletStats.total_earned + walletStats.total_spent
  ...
  const nextLevel =
    SCORE_LEVELS[Math.min(currentLevel.index + 1, SCORE_LEVELS.length - 1)];
  const progressInLevel = totalTCC - currentLevel.min;
  const levelRange =
    currentLevel.max === Infinity ? 10000 : currentLevel.max - currentLevel.min;
  ...
  const getShopName = (tx: Transaction) => {
    ...
    if (tx.type === "referral") {
      if (
        tx.description.includes("registrato") ||
        tx.description.includes("invito")
      )
        return "Invito Amico";
      if (tx.description.includes("primo acquisto"))
        return "Bonus Primo Acquisto";
          ...
          <Card className="w-full max-w-sm text-center">
            <CardContent className="pt-6">
              <p className="text-muted-foreground mb-4">
                Devi accedere per vedere lo storico
              </p>
              <Link href="/wallet">
      ...
      <header className="bg-gradient-to-r from-primary to-emerald-600 text-white p-3 sm:p-4 flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <div>
          ...
          <p className="text-xs text-white/70">
            {walletStats?.total_transactions || transactions.length} transazioni
          </p>
        </div>
      </header>
      ...
      <div className="shrink-0 p-3 sm:p-4 grid grid-cols-2 gap-2 sm:gap-3">
        ...
        <Card
          ...
          style={{
            ...
            `,
            boxShadow:
              "0 10px 40px -10px rgba(16, 185, 129, 0.6), inset 0 1px 0 rgba(255,255,255,0.1)",
          }}
        >
            ...
            <div className="flex items-center gap-2 mb-2">
              <div
                ...
                style={{
                  ...
                  boxShadow:
                    "0 4px 15px rgba(16, 185, 129, 0.5), inset 0 1px 0 rgba(255,255,255,0.3)",
                }}
              >
                ...
                <>
                  <div>
                    <p className="text-2xl sm:text-3xl font-black text-white drop-shadow-lg">
                      €
                      {lastTx.euro_value
                        ? (lastTx.euro_value / 100).toFixed(2)
                        : "0.00"}
                    </p>
                    <p
                  ...
                  <div className="pt-2 border-t border-emerald-400/30">
                    ...
                    <p className="text-sm text-emerald-200 flex items-center gap-1">
                      🌳{" "}
                      <span className="font-bold text-white">
                        {(Math.abs(lastTx.amount) / 22).toFixed(1)}
                      </span>{" "}
                      alberi
                    </p>
                  </div>
