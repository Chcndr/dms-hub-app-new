...
function getClientSidePermissions(isImpersonating: boolean): Permission[] {
  ...
  try {
    ...
    if (isMobileImpresaTestMode()) {
      console.warn(
        "[PermissionsContext] MIO TEST: modalita mobile-impresa per",
        user.email
      );
      for (const code of IMPRESA_ONLY_PERMISSION_CODES) {
        extraPerms.push({
          ...
          code,
          name: code,
          category: code.startsWith("tab") ? "tab" : "quick",
          is_sensitive: false,
        });
      }
      return extraPerms;
...
async function determineUserRoleId(): Promise<{
  roleId: number;
  isImpersonating: boolean;
}> {
  ...
  if (impersonation.isImpersonating && impersonation.comuneId) {
    console.warn(
      "[PermissionsContext] Modalita impersonificazione COMUNE - usando ruolo admin_pa"
    );
    return { roleId: ROLE_IDS.ADMIN_PA, isImpersonating: true };
  }
  if (impersonation.isImpersonating && impersonation.associazioneId) {
    console.warn(
      "[PermissionsContext] Modalita impersonificazione ASSOCIAZIONE - usando ruolo association (ID=10)"
    );
  ...
  if (!userStr) {
    console.warn(
      "[PermissionsContext] Nessun utente loggato - usando ruolo citizen"
    );
  ...
  try {
    ...
    if (user.assigned_roles && user.assigned_roles.length > 0) {
      const nonCitizenRole = user.assigned_roles.find(
        (r: { role_id: number; role_code: string }) =>
          r.role_id !== ROLE_IDS.CITIZEN
      );
      if (nonCitizenRole) {
        console.warn(
          `[PermissionsContext] Utente con ruolo assegnato: ${nonCitizenRole.role_code} (ID=${nonCitizenRole.role_id})`
        );
    ...
    console.warn(
      `[PermissionsContext] Utente standard (base_role=${user.base_role || "none"}) - usando ruolo citizen`
    );
    return { roleId: ROLE_IDS.CITIZEN, isImpersonating: false };
  } catch (err) {
...
export function PermissionsProvider({ children }: PermissionsProviderProps) {
  ...
  const loadUserPermissions = useCallback(async () => {
    ...
    try {
      ...
      const { roleId, isImpersonating: impersonating } =
        await determineUserRoleId();
      setUserRoleId(roleId);
      setIsImpersonating(impersonating);

      console.warn(
        `[PermissionsContext] Caricamento permessi per ruolo ID=${roleId}`
      );
      ...
      if (data.success && data.data && data.data.permissions) {
        ...
        console.warn(
          `[PermissionsContext] Caricati ${serverPerms.length} permessi dal server`
        );
      } else if (data.success && Array.isArray(data.data)) {
        serverPerms = data.data;
        console.warn(
          `[PermissionsContext] Caricati ${serverPerms.length} permessi (formato legacy)`
        );
      } else {
      ...
      if (extraPerms.length > 0) {
        console.warn(
          `[PermissionsContext] Aggiunti ${extraPerms.length} permessi client-side: ${extraPerms.map(p => p.code).join(", ")}`
        );
      }
      ...
      if (isMobileImpresaTestMode()) {
        ...
        console.warn(
          `[PermissionsContext] MIO TEST: filtrati a ${allPerms.length} permessi impresa`
        );
      }
      ...
      if (clientPerms.length > 0) {
        console.warn(
          `[PermissionsContext] Fallback: usando ${clientPerms.length} permessi client-side`
        );
    ...
    if (userStr) {
      try {
        ...
        sessionStorage.setItem(
          "_perms_last_user_role",
          `${!!u.is_super_admin}:${u.base_role || ""}`
        );
      } catch {
        /* ignore */
      }
  ...
  const canViewQuickAccess = useCallback(
    (quickId: string): boolean => {
