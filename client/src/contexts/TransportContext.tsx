...
const transformApiStop = (apiStop: any): TransportStop => {
    ...
    transformedRoutes = apiStop.routes.map((r: any) => {
      ...
      if (typeof r === "object" && r !== null) {
        ...
        return {
          ...
          route_long_name: locationName
            ? `Linea ${lineId} - ${locationName}`
            : `Linea ${lineId}`,
          route_type: apiStop.stop_type === "train" ? 2 : 3,
        };
      }
  ...
  return {
    ...
    agency_name:
      apiStop.provider === "tper"
        ? "TPER"
        : apiStop.provider === "trenitalia"
          ? "Trenitalia"
          : apiStop.provider === "tiemme"
            ? "Tiemme"
            : apiStop.provider,
...
export function TransportProvider({ children }: TransportProviderProps) {
  ...
  const loadStops = useCallback(async (region?: string) => {
    ...
    try {
      ...
      if (allStops.length > 0) {
        setStops(allStops);
        console.warn(
          "[TransportContext] Totale fermate caricate:",
          allStops.length
        );
      } else {
  ...
  const loadStopsNearPoint = useCallback(
    async (
      lat: number,
      lng: number,
      radiusKm: number = 2
      ...
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          console.warn(
            "[TransportContext] Trovate",
            transformedStops.length,
            "fermate vicine"
          );
        ...
        console.error(
          "[TransportContext] Errore caricamento fermate vicine:",
          err
        );
  ...
  const getNextDepartures = useCallback(
    async (stopId: string, limit: number = 10): Promise<StopTime[]> => {
      // TODO: Implementare quando avremo i dati degli orari
      console.warn(
        "[TransportContext] getNextDepartures non ancora implementato"
      );
      return [];
    },
    []
  ...
  const getRoutesForStop = useCallback(
    async (stopId: string): Promise<TransportRoute[]> => {
    ...
    return allStops
      ...
      .map(stop => ({
        ...
        distance_m: Math.round(
          calculateDistance(lat, lng, stop.stop_lat, stop.stop_lon) * 1000
        ),
        walk_time_min: Math.round(
          calculateDistance(lat, lng, stop.stop_lat, stop.stop_lon) * 12
        ),
      }))
        ...
        routes: [
          {
            route_id: "RV",
            route_short_name: "RV",
            route_long_name: "Regionale Veloce",
            route_type: 2,
          },
        ],
      },
        ...
        routes: [
          {
            route_id: "11",
            route_short_name: "11",
            route_long_name: "Stazione - Pilastro",
            route_type: 3,
          },
        ],
      },
