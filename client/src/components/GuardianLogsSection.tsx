...
export default function GuardianLogsSection() {
  ...
  const stats = statsData?.stats
    ? {
        ...
        warn:
          (statsData.stats as any).warnings ||
          Math.max(
            0,
            statsData.stats.total -
              statsData.stats.successful -
              statsData.stats.failed
          ),
  ...
  const systemLogs = guardianLogs.filter(
    (log: any) => log.agent === "system" || log.agent === "anonymous"
  );
  const guardianOnlyLogs = guardianLogs.filter(
    (log: any) =>
      log.agent === "guardian" ||
      log.agent === "mio" ||
      log.agent === "dev" ||
      ...
      <Tabs defaultValue="health" className="w-full">
              ...
              {error ? (
                ...
                <div className="text-center py-8 text-[#e8fbff]/60">
                  ...
                  <p className="text-sm text-[#e8fbff]/40">
                    I log verranno creati automaticamente quando si usano le
                    API.
                  </p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex items-start justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-xs font-semibold px-2 py-0.5 rounded border ${getLevelBadge(log.success)}`}
                          >
        ...
        <TabsContent value="imprese" className="mt-4">
          <Card className="bg-[#1a2332] border-[#14b8a6]/30">
            ...
            <CardContent>
              {(() => {
                const impreseLogs = guardianLogs.filter(
                  (log: any) =>
                ...
                return impreseLogs.length === 0 ? (
                  <div className="text-center py-8 text-[#e8fbff]/60">
                    <p className="mb-2">
                      Nessun log per Imprese & Qualificazioni API.
                    </p>
                    <p className="text-xs text-[#e8fbff]/40">
                      I log appariranno quando verranno effettuate chiamate agli
                      endpoint:
                    </p>
