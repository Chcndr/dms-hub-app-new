...
export default function GuardianDebugSection() {
  ...
  const stats = {
    ...
    warnings: allLogs.filter(
      (log: any) =>
        log.level === "warn" || log.message?.toLowerCase().includes("warning")
    ).length,
  ...
  const getSuggestion = (log: any): string => {
    if (
      log.message?.includes("Unknown agent") ||
      log.message?.includes("not authorized")
    ) {
      return "Aggiungi questo agente in agents/permissions.json con i permessi appropriati";
    }
    if (
      log.message?.includes("Unknown endpoint") ||
      ...
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-red-400 font-semibold mb-2">
            Errore nel caricamento dei log di debug
          </p>
      ...
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-[#e8fbff]/60">
            Caricamento debug info dal database...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-[#e8fbff]">
            Guardian Debug & Error Monitoring
          </h3>
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto">
                {errorLogs.map((log: any, idx: number) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {log.serviceId && (
                          <>
                            ...
                            <span className="text-[#e8fbff]/50 text-sm">
                              {log.serviceId}
                            </span>
                          </>
                        )}
                    ...
                    <p className="text-[#e8fbff] text-sm mb-2 font-mono">
                      {log.message}
                    </p>
                    ...
                    <div className="mt-3 p-3 bg-[#0a1628] rounded-lg border border-[#14b8a6]/20">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <p className="text-[#14b8a6] font-semibold text-xs mb-1">
                            Suggerimento:
                          </p>
                          <p className="text-[#e8fbff]/80 text-xs">
                            {getSuggestion(log)}
                          </p>
                        </div>
                      </div>
