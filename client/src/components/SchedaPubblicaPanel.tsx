...
export default function SchedaPubblicaPanel() {
  ...
  const addBeneficio = () =>
    setScheda(s => ({ ...s, benefici: [...s.benefici, ""] }));
  const removeBeneficio = (i: number) =>
    setScheda(s => ({
      ...s,
      benefici: s.benefici.filter((_, idx) => idx !== i),
    }));
  const updateBeneficio = (i: number, val: string) =>
    setScheda(s => ({
      ...s,
      benefici: s.benefici.map((b, idx) => (idx === i ? val : b)),
    }));

  const addSede = () =>
    setScheda(s => ({
      ...s,
      sedi: [...s.sedi, { nome: "", indirizzo: "", orari: "" }],
    }));
  const removeSede = (i: number) =>
    setScheda(s => ({ ...s, sedi: s.sedi.filter((_, idx) => idx !== i) }));
  const updateSede = (
    i: number,
    field: keyof SchedaPubblica["sedi"][0],
    val: string
  ) =>
    setScheda(s => ({
      ...s,
      sedi: s.sedi.map((sede, idx) =>
        idx === i ? { ...sede, [field]: val } : sede
      ),
    }));
        ...
        <TabsContent value="preview">
          <Card className="bg-[#1a2332] border-[#3b82f6]/30">
            <CardContent className="pt-6 space-y-6">
              ...
              {/* Descrizione */}
              <div>
                <h4 className="text-sm font-medium text-[#e8fbff]/70 mb-2">
                  Chi siamo
                </h4>
                <p className="text-sm text-[#e8fbff]">
                  {scheda.descrizione || (
                    <span className="text-[#e8fbff]/30 italic">
                      Nessuna descrizione. Clicca "Modifica" per aggiungerne
                      una.
                    </span>
                  )}
                </p>
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {scheda.benefici.map((b, i) => (
                      <li
                        key={i}
                        className="flex items-center gap-2 text-sm text-[#e8fbff]"
                      >
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {scheda.sedi.map((s, i) => (
                      <div
                        key={i}
                        className="p-3 bg-[#0b1220] rounded-lg border border-[#3b82f6]/10"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {scheda.contatti.referente_nome && (
                      <div className="flex items-center gap-2 text-sm text-[#e8fbff]">
                        ...
                        {scheda.contatti.referente_nome}{" "}
                        {scheda.contatti.referente_ruolo &&
                          `(${scheda.contatti.referente_ruolo})`}
                      </div>
                    )}
                    {scheda.contatti.telefono && (
            ...
            <CardContent className="pt-6 space-y-4">
              <div>
                ...
                <textarea
                  ...
                  onChange={e =>
                    setScheda(s => ({ ...s, descrizione: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    setScheda(s => ({ ...s, logo_url: e.target.value }))
                  }
                />
              </div>
            </CardContent>
          ...
          <Card className="bg-[#1a2332] border-[#3b82f6]/30">
            <CardContent className="pt-6 space-y-3">
              ...
              {!editMode && scheda.benefici.length === 0 && (
                <p className="text-sm text-[#e8fbff]/30 italic text-center py-4">
                  Nessun beneficio configurato
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#1a2332] border-[#3b82f6]/30">
            <CardContent className="pt-6 space-y-4">
              ...
              {scheda.sedi.map((s, i) => (
                <div
                  key={i}
                  className="p-3 bg-[#0b1220] rounded-lg border border-[#3b82f6]/10 space-y-2"
                >
                  <div className="flex items-center justify-between">
                    ...
                    {editMode && (
                      <Button
                        ...
                        className="text-[#ef4444] hover:text-[#ef4444]/80 h-6 w-6"
                      >
                  ...
                  <Input
                    className="bg-[#1a2332] border-[#3b82f6]/20 text-[#e8fbff]"
                    disabled={!editMode}
                  ...
                  <Input
                    className="bg-[#1a2332] border-[#3b82f6]/20 text-[#e8fbff]"
                    disabled={!editMode}
                    placeholder="Orari (es: Lun-Ven 9-17)"
                    value={s.orari}
                    onChange={e => updateSede(i, "orari", e.target.value)}
                  />
                </div>
              ))}
              {editMode && (
              ...
              <h4 className="text-sm font-medium text-[#e8fbff]/70 mt-6">
                Contatti per nuove adesioni
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    value={scheda.contatti.referente_nome || ""}
                    onChange={e =>
                      setScheda(s => ({
                        ...s,
                        contatti: {
                          ...s.contatti,
                          referente_nome: e.target.value,
                        },
                      }))
                    }
                  />
                ...
                <div>
                  ...
                  <Input
                    className="mt-1 bg-[#0b1220] border-[#3b82f6]/20 text-[#e8fbff]"
                    disabled={!editMode}
                    placeholder="Es: Segretario"
                    value={scheda.contatti.referente_ruolo || ""}
                    onChange={e =>
                      setScheda(s => ({
                        ...s,
                        contatti: {
                          ...s.contatti,
                          referente_ruolo: e.target.value,
                        },
                      }))
                    }
                  />
                ...
                <div>
                  ...
                  <Input
                    ...
                    disabled={!editMode}
                    placeholder="+39..."
                    value={scheda.contatti.telefono || ""}
                    onChange={e =>
                      setScheda(s => ({
                        ...s,
                        contatti: { ...s.contatti, telefono: e.target.value },
                      }))
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    disabled={!editMode}
                    placeholder="info@..."
                    value={scheda.contatti.email || ""}
                    onChange={e =>
                      setScheda(s => ({
                        ...s,
                        contatti: { ...s.contatti, email: e.target.value },
                      }))
                    }
                  />
                </div>
