  ...
  return (
    <Card className={`bg-[#0a0f1a] ${config.borderColor}`}>
      ...
      <CardContent className="space-y-3">
        ...
        <div
          ref={containerRef}
          className="h-64 bg-[#0b1220] rounded-lg p-3 overflow-y-scroll space-y-2 multi-agent-chat-container"
        >
          {messages.length === 0 ? (
                ...
                >
                  <div className="text-xs text-[#e8fbff]/50 mb-1">
                    {msg.role === "user"
                      ? msg.sender === "user"
                        ? "Tu"
                        : "MIO" // 🔥 FIX: Mostra MIO quando orchestrator delega, Tu quando utente invia
                      : config.name}
                  </div>
                  <p className="text-xs text-[#e8fbff] whitespace-pre-wrap">
                    {msg.content}
                  </p>
