...
export default function GestioneServiziAssociazionePanel() {
  ...
  const deleteServizio = async (id: number) => {
    ...
    try {
      const res = await authenticatedFetch(
        `${API_BASE}/api/associazioni/${associazioneId}/servizi/${id}`,
        { method: "DELETE" }
      );
      const data = await res.json();
      if (data.success) {
  ...
  const updateStatoRichiesta = async (
    rid: number,
    stato: RichiestaServizio["stato"]
  ) => {
    if (!associazioneId) return;
    try {
      const res = await authenticatedFetch(
        `${API_BASE}/api/associazioni/${associazioneId}/richieste-servizi/${rid}/stato`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        toast.success(
          `Richiesta ${stato === "completata" ? "completata" : stato === "rifiutata" ? "rifiutata" : "aggiornata"}`
        );
        loadData();
      }
    } catch (error) {
      ...
      <Card className="bg-[#1a2332] border-[#3b82f6]/30">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-[#e8fbff] flex items-center gap-2">
            ...
            {richiesteInAttesa > 0 && (
              <Badge className="bg-[#ef4444] text-white ml-2">
                {richiesteInAttesa} nuove
              </Badge>
            )}
          </CardTitle>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-[#0b1220] border border-[#3b82f6]/20">
          ...
          >
            ...
            {richiesteInAttesa > 0 && (
              <Badge className="bg-[#ef4444] text-white ml-1 text-[10px] h-4">
                {richiesteInAttesa}
              </Badge>
            )}
          </TabsTrigger>
            ...
            <Card className="bg-[#1a2332] border-[#10b981]/30 mb-4">
              <CardContent className="pt-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setForm(f => ({
                          ...f,
                          prezzo_base: parseFloat(e.target.value) || 0,
                        }))
                      }
                    />
                  </div>
                  <div>
                    <Label className="text-xs text-[#e8fbff]/50">
                      Prezzo Associati (EUR)
                    </Label>
                    <Input
                      type="number"
                      className="mt-1 bg-[#0b1220] border-[#3b82f6]/20 text-[#e8fbff]"
                      value={form.prezzo_associati}
                      onChange={e =>
                        setForm(f => ({
                          ...f,
                          prezzo_associati: parseFloat(e.target.value) || 0,
                        }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      className="mt-1 bg-[#0b1220] border-[#3b82f6]/20 text-[#e8fbff]"
                      value={form.tempo_medio_gg}
                      onChange={e =>
                        setForm(f => ({
                          ...f,
                          tempo_medio_gg: parseInt(e.target.value) || 0,
                        }))
                      }
                    />
                  </div>
              ...
              {servizi.length === 0 ? (
                <div className="text-center py-8 text-[#e8fbff]/50">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={openNew}
                    className="mt-3 border-[#3b82f6]/30 text-[#3b82f6]"
                  >
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-[#e8fbff]/40 mt-1">
                            {new Date(r.data_richiesta).toLocaleDateString(
                              "it-IT"
                            )}
                            {r.note && ` · ${r.note}`}
                          </p>
                        ...
                        <div className="flex items-center gap-2">
                          <Badge
                            style={{
                              backgroundColor: `${STATO_COLORS[r.stato]}20`,
                              color: STATO_COLORS[r.stato],
                              borderColor: `${STATO_COLORS[r.stato]}30`,
                            }}
                          >
                            {r.stato.replace("_", " ")}
                          </Badge>
                          {r.stato === "in_attesa" && (
                            <>
                              <Button
                                size="sm"
                                onClick={() =>
                                  updateStatoRichiesta(r.id, "in_lavorazione")
                                }
                              ...
                              <Button
                                ...
                                variant="outline"
                                onClick={() =>
                                  updateStatoRichiesta(r.id, "rifiutata")
                                }
                          ...
                          {r.stato === "in_lavorazione" && (
                            <Button
                              size="sm"
                              onClick={() =>
                                updateStatoRichiesta(r.id, "completata")
                              }
