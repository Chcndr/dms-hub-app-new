...
export default function ConnessioniV2() {
  ...
  const [lastSyncInfo, setLastSyncInfo] = useState<
    Record<string, { timestamp: string; status: string; details?: any }>
  >({});
  ...
  const handleHealthCheck = async (integration: IntegrationConfig) => {
    ...
    try {
      if (integration.id === "dms-legacy") {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/integrations/mercaweb/health`,
          {
            headers: {
  ...
  const handleMercaWebStatus = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/integrations/mercaweb/status`,
        {
          headers: {
            "X-MercaWeb-API-Key": import.meta.env.VITE_MERCAWEB_API_KEY || "",
          },
      ...
      if (data.success) {
        ...
        setLastSyncInfo(prev => ({
          ...
          mercaweb: {
            timestamp:
              data.data.lastImport?.timestamp || new Date().toISOString(),
  ...
  const handleSync = async (integration: IntegrationConfig) => {
    ...
    try {
      const response = await authenticatedFetch(
        `${API_BASE_URL}/api/integrations/dms-legacy/sync`,
        {
          method: "POST",
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-200/70">
                Questo pannello mostra tutte le integrazioni esterne del sistema
                DMS HUB. Ogni integrazione ha un "Data Owner" che indica chi è
                il master dei dati per quella specifica area.
              </p>
            </div>
          </div>
        </CardContent>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-400">
                  {
                    integrations.filter(i => i.status === "in_preparation")
                      .length
                  }
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {integrations.map(integration => (
          <Card
            key={integration.id}
            className="bg-[#1a2332] border-[#14b8a6]/30"
          >
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-start gap-2 text-sm">
                ...
                <div>
                  ...
                  <span className="text-[#e8fbff] ml-2 font-semibold">
                    {integration.dataOwner}
                  </span>
                </div>
              </div>
              ...
              {integration.id === "mercaweb" && lastSyncInfo["mercaweb"] && (
                <div className="bg-green-500/10 p-3 rounded-lg border border-green-500/20">
                  ...
                  <p className="text-xs text-green-200/70">
                    {lastSyncInfo["mercaweb"].timestamp
                      ? new Date(
                          lastSyncInfo["mercaweb"].timestamp
                  ...
                  {lastSyncInfo["mercaweb"].details && (
                    <div className="mt-1 flex gap-3 text-xs text-green-200/60">
                      <span>
                        Mercati MW:{" "}
                        {lastSyncInfo["mercaweb"].details.markets
                          ?.with_mercaweb_id || 0}
                      </span>
                      <span>
                        Imprese MW:{" "}
                        {lastSyncInfo["mercaweb"].details.imprese
                          ?.with_mercaweb_id || 0}
                      </span>
                      <span>
                        Concessioni MW:{" "}
                        {lastSyncInfo["mercaweb"].details.concessions
                          ?.with_mercaweb_id || 0}
                      </span>
                    </div>
                  )}
              ...
              {integration.id === "dms-legacy" &&
                lastSyncInfo["dms-legacy"] && (
                  <div className="bg-green-500/10 p-3 rounded-lg border border-green-500/20">
                    ...
                    <p className="text-xs text-green-200/70">
                      {new Date(
                        lastSyncInfo["dms-legacy"].timestamp
                      ).toLocaleString("it-IT")}{" "}
                    ...
                    {lastSyncInfo["dms-legacy"].details && (
                      <div className="mt-1 flex gap-3 text-xs text-green-200/60">
                        <span>
                          Mercati:{" "}
                          {lastSyncInfo["dms-legacy"].details.markets?.count ||
                            0}
                        </span>
                        <span>
                          Ambulanti:{" "}
                          {lastSyncInfo["dms-legacy"].details.vendors?.count ||
                            0}
                        </span>
                        <span>
                          Concessioni:{" "}
                          {lastSyncInfo["dms-legacy"].details.concessions
                            ?.count || 0}
                        </span>
                      </div>
                    )}
              ...
              <div className="bg-[#0a1628] p-3 rounded-lg border border-[#14b8a6]/20">
                <p className="text-xs text-[#e8fbff]/70 mb-1 font-semibold">
                  Note Tecniche:
                </p>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {integration.endpoints.map((endpoint, idx) => (
                      ...
                      >
                        <code className="text-xs text-[#14b8a6] font-mono">
                          {endpoint}
                        </code>
                      </div>
                    ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-[#0a1628] p-4 rounded-lg border border-[#14b8a6]/20">
              ...
              <p className="text-xs text-[#e8fbff]/70">
                Master dei dati mercati Bologna/Cervia e app di spunta.
                Integrazione attiva via API Proxy (Lapsy srl - Heroku).
              </p>
            </div>
            ...
            <div className="bg-[#0a1628] p-4 rounded-lg border border-purple-500/20">
              ...
              <p className="text-xs text-[#e8fbff]/70">
                Integrazione bidirezionale per anagrafiche e presenze. Usato
                dalla Polizia Municipale di Grosseto.
              </p>
            </div>
            ...
            <div className="bg-[#0a1628] p-4 rounded-lg border border-[#14b8a6]/20">
              ...
              <p className="text-xs text-[#e8fbff]/70">
                Master delle geometrie e coordinate dei posteggi. File sorgente:
                editor-v3-full.json.
              </p>
            </div>
            ...
            <div className="bg-[#0a1628] p-4 rounded-lg border border-[#14b8a6]/20">
              ...
              <p className="text-xs text-[#e8fbff]/70">
                Master dei dati mobilità (fermate, linee, orari). API esterne da
                integrare.
              </p>
            </div>
          </div>
