    ...
    const loadSaldo = async () => {
      try {
        ...
        if (data.success && Array.isArray(data.data) && data.data.length > 0) {
          ...
          setSaldo(
            wallet ? parseFloat(wallet.balance || wallet.saldo || "0") || 0 : 0
          );
        } else if (
          data.success &&
          ...
          setSaldo(
            parseFloat(data.data.balance || data.data.saldo || "0") || 0
          );
        } else {
          setSaldo(0);
        }
        ...
        <CardContent className="p-5 space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-[#e8fbff]/50">
                  Pagamento da wallet generico
                </p>
              </div>
            </div>
            <button
              onClick={onClose}
              className="text-[#e8fbff]/40 hover:text-[#e8fbff]"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {paid ? (
            <div className="text-center py-6">
              ...
              <p className="text-lg font-semibold text-[#e8fbff]">
                Pagamento effettuato
              </p>
              <p className="text-sm text-[#e8fbff]/60 mt-1">{descrizione}</p>
              <Button
                onClick={onClose}
                className="mt-4 bg-[#14b8a6] hover:bg-[#14b8a6]/80 text-white"
              >
                Chiudi
              </Button>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#0b1220]/50 rounded-lg p-3 text-center border border-[#14b8a6]/10">
                  ...
                  <p
                    className={`text-xl font-bold ${(saldo || 0) >= importo ? "text-[#10b981]" : "text-red-400"}`}
                  >
              ...
              {(saldo || 0) >= importo ? (
                ...
                >
                  ...
                  {paying
                    ? "Pagamento in corso..."
                    : `Conferma Pagamento - €${importo.toFixed(2)}`}
                </Button>
              ) : (
                  ...
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#e8fbff]/50 mt-1">
                        Servono &euro;{(importo - (saldo || 0)).toFixed(2)} in
                        piu'. Ricarica il wallet per procedere.
                      </p>
                    </div>
                  </div>
                  <Button
