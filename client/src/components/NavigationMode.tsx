...
export function NavigationMode({
  ...
  const fetchRoute = useCallback(
    async (start: { lat: number; lng: number }) => {
      try {
        const profile =
  ...
  const getManeuverText = (
    ...
    if (!text) {
      ...
      if (type === "fork" && modifier === "slight right")
        text = "↗ Tieni leggermente a destra al bivio";
      else if (type === "fork" && modifier === "slight left")
        text = "↖ Tieni leggermente a sinistra al bivio";
      ...
      <Card className="bg-slate-900/95 backdrop-blur-md border-slate-700 p-3 sm:p-4">
        ...
        {arrived ? (
          <div className="bg-emerald-500/20 border border-emerald-500/30 rounded-lg p-3 sm:p-4 text-center">
            ...
            <p className="text-emerald-400 font-bold text-base sm:text-lg mt-2">
              Sei arrivato!
            </p>
          ...
          <div className="bg-red-500/20 border border-red-500/30 rounded-lg p-2 sm:p-3">
            ...
            <p className="text-slate-400 text-xs mt-1">
              Verifica le impostazioni GPS del dispositivo
            </p>
          </div>
        ) : gpsLoading ? (
          ...
          <div className="bg-slate-800/50 rounded-lg p-2 sm:p-3">
            <p className="text-white text-lg sm:text-xl font-bold">
              🚀{" "}
              {instructions[currentStep].text.includes("depart") ||
              instructions[currentStep].type === "depart"
                ? "Parti"
  ...
  return (
    <>
      ...
      {overlayContainer &&
        createPortal(<NavigationOverlayUI />, overlayContainer)}
    </>
  );
}
