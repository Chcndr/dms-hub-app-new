...
export default function ConcessioneForm({
  onCancel,
  onSubmit,
  initialData,
  mode = "create",
  concessioneId,
}: ConcessioneFormProps) {
    ...
    if (initialData) {
      ...
      const formattedData = {
        ...
        data_protocollazione:
          formatDate(initialData.data_protocollazione) ||
          formatDate(initialData.valid_from) ||
          new Date().toISOString().split("T")[0],
        data_decorrenza:
          formatDate(initialData.data_decorrenza) ||
          formatDate(initialData.valid_from) ||
          new Date().toISOString().split("T")[0],
        data_scadenza:
          formatDate(initialData.data_scadenza) ||
          formatDate(initialData.valid_to) ||
          "",
        data_nascita: formatDate(initialData.data_nascita),
        autorizzazione_precedente_data: formatDate(
          initialData.autorizzazione_precedente_data
        ),
      };

      // Calcola durata e scadenza
      const durataAnni = formattedData.durata_anni || "10";
      const dataDecorrenza =
        formattedData.data_decorrenza || new Date().toISOString().split("T")[0];
      ...
      setFormData(prev => ({
        ...
        tipo_concessione:
          formattedData.tipo_concessione ||
          prev.tipo_concessione ||
          "subingresso",
      }));
      ...
      toast.info("Form pre-compilato con i dati della concessione", {
        ...
        try {
          ...
          if (concessionsJson.success && concessionsJson.data) {
            ...
            const maxId = Math.max(
              ...concessionsJson.data.map((c: any) => c.id),
              0
            );
          ...
          console.warn(
            "[ConcessioneForm] Errore nel caricare il prossimo numero:",
            err
          );
        }
            ...
            if (targetMarket) {
              ...
              if (initialData?.mercato) {
                const marketByName = marketsJson.data.find((m: Market) =>
                  m.name
                    .toLowerCase()
                    .includes(String(initialData.mercato).toLowerCase())
                );
                if (marketByName) {
          ...
          if (initialData?.cf_concessionario || initialData?.partita_iva) {
            ...
            if (foundImpresa) {
              ...
              setFormData(prev => ({
                ...prev,
                cf_concessionario:
                  foundImpresa.codice_fiscale || prev.cf_concessionario,
                partita_iva: foundImpresa.partita_iva || prev.partita_iva,
                ragione_sociale:
                  foundImpresa.denominazione || prev.ragione_sociale,
                nome: foundImpresa.rappresentante_legale_nome || prev.nome,
                cognome:
                  foundImpresa.rappresentante_legale_cognome || prev.cognome,
                data_nascita: foundImpresa.rappresentante_legale_data_nascita
                  ? foundImpresa.rappresentante_legale_data_nascita.split(
                      "T"
                    )[0]
                  : prev.data_nascita,
                luogo_nascita:
                  foundImpresa.rappresentante_legale_luogo_nascita ||
                  prev.luogo_nascita,
                residenza_via: foundImpresa.rappresentante_legale_residenza_via
                  ? `${foundImpresa.rappresentante_legale_residenza_via} ${foundImpresa.rappresentante_legale_residenza_civico || ""}`.trim()
                  : prev.residenza_via,
                residenza_comune:
                  foundImpresa.rappresentante_legale_residenza_comune ||
                  foundImpresa.comune ||
                  prev.residenza_comune,
                residenza_provincia:
                  foundImpresa.rappresentante_legale_residenza_provincia ||
                  foundImpresa.indirizzo_provincia ||
                  prev.residenza_provincia,
                residenza_cap:
                  foundImpresa.rappresentante_legale_residenza_cap ||
                  foundImpresa.indirizzo_cap ||
                  prev.residenza_cap,
                sede_legale_via: foundImpresa.indirizzo_via
                  ? `${foundImpresa.indirizzo_via} ${foundImpresa.indirizzo_civico || ""}`.trim()
                  : prev.sede_legale_via,
                sede_legale_comune:
                  foundImpresa.comune || prev.sede_legale_comune,
                sede_legale_provincia:
                  foundImpresa.indirizzo_provincia ||
                  prev.sede_legale_provincia,
                sede_legale_cap:
                  foundImpresa.indirizzo_cap || prev.sede_legale_cap,
              }));
      ...
      try {
        ...
        if (json.success && json.data) {
          ...
          if (initialData?.posteggio_id || initialData?.posteggio) {
            const targetStall = sortedStalls.find(
              (s: Stall) =>
            ...
            if (targetStall) {
              ...
              if (
                ...
                const canoneAnnuo =
                  parseFloat(targetStall.area_mq) *
              ...
              if (canone) {
                toast.success(
                  `Posteggio ${targetStall.number} pre-selezionato`,
                  {
  ...
  const selectImpresa = (impresa: Impresa) => {
    setFormData(prev => ({
      ...
      residenza_comune:
        impresa.rappresentante_legale_residenza_comune || impresa.comune || "",
      residenza_provincia:
        impresa.rappresentante_legale_residenza_provincia ||
        impresa.indirizzo_provincia ||
        "",
      residenza_cap:
        impresa.rappresentante_legale_residenza_cap ||
        impresa.indirizzo_cap ||
        "",
      // Sede Legale
    ...
    toast.success("Concessionario selezionato!", {
  ...
  const handleStallChange = (stallId: string) => {
    ...
    if (stall) {
      ...
      // Preserva la merceologia da initialData se presente
      const merceologia =
        formData.merceologia || initialData?.merceologia || "";
      ...
      setFormData(prev => ({
        ...
        mq: stall.area_mq || "",
        dimensioni_lineari:
          stall.dimensions || `${stall.width} x ${stall.depth}`,
        tipo_posteggio: "fisso", // Concessione è sempre fisso, mai spunta
        merceologia: merceologia,
        canone_unico: canone,
      }));
  ...
  const calculateExpiry = (years: string) => {
    const baseDate =
      formData.data_decorrenza ||
      formData.data_protocollazione ||
      new Date().toISOString().split("T")[0];
    const date = new Date(baseDate);
    if (isNaN(date.getTime())) {
      console.warn(
        "[ConcessioneForm] Invalid date for calculateExpiry:",
        baseDate
      );
      return;
    }
  ...
  const handleDecorrenzaChange = (dataDecorrenza: string) => {
    ...
    if (isNaN(date.getTime())) {
      console.warn(
        "[ConcessioneForm] Invalid date for handleDecorrenzaChange:",
        dataDecorrenza
      );
      return;
    }
    date.setFullYear(
      date.getFullYear() + parseInt(formData.durata_anni || "10")
    );
    setFormData(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const effectiveMarketId =
      selectedMarketId ||
      (mode === "edit"
        ? Number(initialData?.mercato_id || initialData?.market_id)
        : null);
    const effectiveStallId =
      selectedStallId ||
      (mode === "edit"
    ...
    try {
      ...
      const dataToSave = {
        ...
        limitazioni_merceologia: sanitizeValue(
          formData.limitazioni_merceologia
        ),
        ...
        cedente_ragione_sociale: sanitizeValue(
          formData.cedente_ragione_sociale
        ),
        // Autorizzazione precedente
        autorizzazione_precedente_pg: sanitizeValue(
          formData.autorizzazione_precedente_pg
        ),
        autorizzazione_precedente_data: sanitizeValue(
          formData.autorizzazione_precedente_data
        ),
        autorizzazione_precedente_intestatario: sanitizeValue(
          formData.autorizzazione_precedente_intestatario
        ),
        // SCIA precedente
      ...
      if (!response.ok) {
        ...
        if (!contentType.includes("application/json")) {
          ...
          console.error(
            "[ConcessioneForm] Risposta non-JSON:",
      ...
      if (result.success) {
        ...
        if (result.steps && result.steps.length > 0) {
          result.steps.forEach(
            (step: { tipo: string; msg: string }, index: number) => {
              setTimeout(() => {
          ...
          toast.success(
            isEditMode
              ? "Concessione aggiornata con successo!"
              : "Concessione creata con successo!"
          );
        }
        ...
        const errorMsg =
          result.error ||
          result.message ||
          "Impossibile salvare la concessione";

        if (
          errorMsg.toLowerCase().includes("overlapping") ||
          errorMsg.toLowerCase().includes("conflict")
        ) {
          toast.error("Concessione già esistente per questo posteggio", {
            description:
              "Il posteggio selezionato ha già una concessione attiva. Per un subingresso, la concessione precedente deve essere prima chiusa o scaduta.",
            duration: 8000,
          });
        } else if (errorMsg.toLowerCase().includes("required")) {
          toast.error("Dati mancanti", {
            description:
              "Verifica di aver compilato tutti i campi obbligatori (mercato, posteggio, date)",
            duration: 6000,
          });
        } else {
  ...
  const mostraAutorizzazionePrecedente = [
    "subingresso",
    "conversione",
    "rinnovo",
    "voltura",
  ].includes(formData.tipo_concessione);
    ...
    <Card className="bg-[#0a1628] border-[#1e293b] w-full p-4">
      <CardHeader>
        ...
        <CardDescription className="text-[#e8fbff]/60">
          {mode === "edit"
            ? `Modifica concessione #${concessioneId || initialData?.id}`
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({ ...formData, numero_file: e.target.value })
                  }
                  placeholder="Es. 2"
                  className="bg-[#020817] border-[#1e293b] text-[#e8fbff]"
                />
              ...
              {mostraConversione && (
                <div className="space-y-2">
                  <Label className="text-[#e8fbff]">
                    Sottotipo Conversione *
                  </Label>
                  <Select
                    value={formData.sottotipo_conversione}
                    onValueChange={val =>
                      setFormData({ ...formData, sottotipo_conversione: val })
                    }
                  >
                    <SelectTrigger className="bg-[#020817] border-[#1e293b] text-[#e8fbff]">
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="merceologia">
                        Cambio Merceologia
                      </SelectItem>
                      <SelectItem value="dimensioni">
                        Cambio Dimensioni
                      </SelectItem>
                    </SelectContent>
                  </Select>
          ...
          <div className="space-y-4 border p-4 rounded-lg border-[#1e293b]">
            <h3 className="text-sm font-semibold text-[#e8fbff]">
              Dati Concessionario (Subentrante)
            </h3>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      residenza_comune: e.target.value,
                    })
                  }
                  className="bg-[#020817] border-[#1e293b] text-[#e8fbff]"
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      residenza_provincia: e.target.value.toUpperCase(),
                    })
                  }
                  placeholder="Es. BO"
                  maxLength={2}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      sede_legale_via: e.target.value,
                    })
                  }
                  className="bg-[#020817] border-[#1e293b] text-[#e8fbff]"
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      sede_legale_provincia: e.target.value.toUpperCase(),
                    })
                  }
                  placeholder="Es. BO"
                  maxLength={2}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      sede_legale_cap: e.target.value,
                    })
                  }
                  className="bg-[#020817] border-[#1e293b] text-[#e8fbff]"
                />
          ...
          {mostraCedente && (
            <div className="space-y-4 border p-4 rounded-lg border-[#14b8a6]/30 bg-[#14b8a6]/5">
              <h3 className="text-sm font-semibold text-[#14b8a6]">
                Dati Cedente (Solo per Subingresso)
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-2 relative">
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setCedenteSearchQuery(e.target.value.toUpperCase())
                    }
                    onFocus={() =>
            ...
            <div className="space-y-4 border p-4 rounded-lg border-[#f59e0b]/30 bg-[#f59e0b]/5">
              ...
              {formData.sottotipo_conversione === "merceologia" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label className="text-[#e8fbff]">
                      Merceologia Precedente
                    </Label>
                    <Select
                      value={formData.merceologia_precedente}
                      onValueChange={val =>
                        setFormData({
                          ...formData,
                          merceologia_precedente: val,
                        })
                      }
                    >
                      <SelectTrigger className="bg-[#020817] border-[#1e293b] text-[#e8fbff]">
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="Non Alimentare">
                          Non Alimentare
                        </SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={formData.merceologia_nuova}
                      onValueChange={val =>
                        setFormData({ ...formData, merceologia_nuova: val })
                      }
                    >
                      <SelectTrigger className="bg-[#020817] border-[#1e293b] text-[#e8fbff]">
                        <SelectValue placeholder="Seleziona..." />
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          dimensioni_precedenti: e.target.value,
                        })
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setFormData({ ...formData, mq_nuovi: e.target.value })
                      }
                      className="bg-[#020817] border-[#1e293b] text-[#e8fbff]"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          dimensioni_nuove: e.target.value,
                        })
                      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({ ...formData, tipo_posteggio: e.target.value })
                  }
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={val =>
                    setFormData({ ...formData, merceologia: val })
                  }
                >
                  <SelectTrigger className="bg-[#020817] border-[#1e293b] text-[#e8fbff]">
                    <SelectValue placeholder="Seleziona..." />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    limitazioni_merceologia: e.target.value,
                  })
                }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({ ...formData, canone_unico: e.target.value })
                  }
          ...
          <div className="space-y-4 border p-4 rounded-lg border-[#1e293b]">
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    planimetria_allegata: e.target.checked,
                  })
                }
                className="w-4 h-4"
              />
              <Label htmlFor="planimetria" className="text-[#e8fbff]">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={e =>
                  setFormData({ ...formData, prescrizioni: e.target.value })
                }
          ...
          <div className="flex justify-end gap-4">
            <Button
              ...
              variant="outline"
              onClick={onCancel}
              className="border-[#e8fbff]/20 text-[#e8fbff]"
            >
              Annulla
            </Button>
            <Button
