...
export default function DomandaSpuntaDetail({
  ...
  const handleApprova = async () => {
    if (
      !confirm(
        "Sei sicuro di voler approvare questa domanda? Verrà creato il wallet spunta."
      )
    )
      return;
    ...
    try {
      const res = await authenticatedFetch(
        `${API_URL}/api/domande-spunta/${domandaId}/approva`,
        {
          method: "POST",
  ...
  const handleRevisione = async () => {
    const motivo = prompt(
      "Inserisci il motivo della richiesta di regolarizzazione:"
    );
    if (!motivo) return;
    setActionLoading(true);
    try {
      const res = await authenticatedFetch(
        `${API_URL}/api/domande-spunta/${domandaId}/revisione`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleRifiuta = async () => {
    ...
    try {
      const res = await authenticatedFetch(
        `${API_URL}/api/domande-spunta/${domandaId}/rifiuta`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ motivo }),
  ...
  if (!domanda) {
    return (
      <div className="text-center text-gray-400 py-8">Domanda non trovata</div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {!isAssociazione &&
            domanda.stato !== "APPROVATA" &&
            domanda.stato !== "RIFIUTATA" && (
              <>
                <Button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-[#e8fbff] font-medium">
                {formatDate(domanda.data_richiesta)}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">
            ...
            <div>
              ...
              <p className="text-[#e8fbff] font-medium">
                {domanda.giorno_settimana || domanda.market_days || "-"}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">
            ...
            <div>
              ...
              <p className="text-[#e8fbff] font-medium">
                {formatDate(domanda.data_prima_presenza)}
              </p>
            </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="text-[#e8fbff] font-medium">
                {domanda.rappresentante_legale_nome || "-"}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">
              ...
              <p className="text-[#e8fbff] font-medium">
                {domanda.rappresentante_legale_cognome || "-"}
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-[#e8fbff] font-medium">
                {formatDate(domanda.autorizzazione_data)}
              </p>
            </div>
          </div>
        </CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="text-[#e8fbff] font-medium text-lg">
                <span
                  className={
                    parseFloat(domanda.wallet_balance || 0) > 0
                      ? "text-green-400"
                      : "text-orange-400"
                  }
                >
