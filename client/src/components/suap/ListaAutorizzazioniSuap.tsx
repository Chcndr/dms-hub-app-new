...
export default function ListaAutorizzazioniSuap({
  ...
  const getStatoBadge = (stato: string) => {
    switch (stato?.toUpperCase()) {
      ...
      case "SOSPESA":
        return (
          <Badge className="bg-yellow-500/20 text-yellow-400">Sospesa</Badge>
        );
      case "REVOCATA":
  ...
  const getTipoBadge = (tipo: string) => {
    switch (tipo) {
      case "A":
        return (
          <Badge className="bg-purple-500/20 text-purple-400">Tipo A</Badge>
        );
      case "B":
        return (
          <Badge className="bg-purple-500/20 text-purple-400">Tipo B</Badge>
        );
      default:
        return (
          <Badge className="bg-gray-500/20 text-gray-400">
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-gray-500">
              ...
              <p className="text-sm mt-2">
                Le autorizzazioni create appariranno qui
              </p>
            </div>
          ) : (
            <Table>
                ...
                {filteredAutorizzazioni.map(auth => (
                  ...
                  >
                    ...
                    <TableCell>
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {auth.company_piva || auth.company_cf}
                        </p>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-gray-400">
                      {formatDate(auth.data_rilascio)}
                    </TableCell>
                    <TableCell className="text-gray-400">
                      {formatDate(auth.data_scadenza)}
                    </TableCell>
