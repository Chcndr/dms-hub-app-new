...
export default function StoricoTitolarita({
  ...
  const buildChains = useCallback(
    (eventiList: StoricoEvento[]): PosteggioChain[] => {
  ...
  const loadStoricoPosteggio = useCallback(
    async (posteggioId: number) => {
      setLoading(true);
      try {
  ...
  const getTipoEventoBadge = (tipo: string) => {
    switch (tipo) {
      ...
      default:
        return (
          <Badge className="bg-gray-500/20 text-gray-400 text-xs">{tipo}</Badge>
        );
    }
  };
    ...
    <div className="space-y-4">
      ...
      <Card className="bg-[#1e293b] border-gray-700">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-gray-400">
            Timeline a catena dei cambi di titolarità per ogni posteggio. Dati
            archiviati per graduatorie Bolkestein e documentazione legale.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {selectedPosteggio && (
            <div className="mt-2">
              <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30">
                ...
                Filtro attivo: Posteggio{" "}
                {stalls.find(s => s.id === selectedPosteggio)?.number ||
                  selectedPosteggio}
              </Badge>
            </div>
          )}
      ...
      {loading ? (
        ...
        <Card className="bg-[#1e293b] border-gray-700">
          <CardContent className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-1">
              Gli eventi verranno registrati automaticamente durante i
              subingressi e i rinnovi
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {chains.map(chain => (
            <Card
              key={chain.posteggio_id}
              className="bg-[#1e293b] border-gray-700 overflow-hidden"
            >
                    ...
                    {chain.eventi.map((ev, idx) => (
                      <React.Fragment key={ev.id}>
                        {idx === 0 && ev.cedente_ragione_sociale && (
                          <>
                            <span
                              className="text-gray-500 text-xs max-w-[120px] truncate"
                              title={ev.cedente_ragione_sociale}
                            >
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            >
                              ...
                              <ChevronDown
                                className={`h-4 w-4 text-gray-600 group-hover:text-amber-400 transition-transform ${
                                  expandedEvento === evento.id
                                    ? "rotate-180"
                                    : ""
                                }`}
                            ...
                            {expandedEvento === evento.id && (
                              <div className="mt-3 bg-[#0f172a] rounded-lg p-4 border border-gray-700/50">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <div className="text-sm space-y-1.5">
                                      ...
                                      <div className="flex justify-between gap-2">
                                        ...
                                        <span className="text-gray-300 text-xs">
                                          #
                                          {evento.concessione_subentrante_id ||
                                            "-"}
                                        </span>
                                      </div>
                                      {evento.scia_cedente_numero && (
                                  ...
                                  <div className="space-y-2">
                                    <h4 className="text-amber-400 font-semibold text-sm flex items-center gap-1">
                                      <History className="h-4 w-4" /> Dati
                                      Archiviati Dante Causa
                                    </h4>
                                    <div className="text-sm space-y-1.5">
                                      <div className="flex justify-between gap-2">
                                        <span className="text-gray-500 text-xs">
                                ...
                                {evento.dati_presenze_cedente_json &&
                                  ...
                                  evento.dati_presenze_cedente_json.length >
                                    0 && (
                                    <div className="mt-4 border-t border-gray-700 pt-3">
                                      <h4 className="text-amber-400 font-semibold text-xs mb-2">
                                        Presenze Archiviate del Dante Causa (
                                        {
                                          evento.dati_presenze_cedente_json
                                            .length
                                        }
                                        ...
                                        <table className="w-full">
                                          <thead>
                                            <tr className="border-b border-gray-700">
                                              <th className="text-left text-gray-500 px-2 py-1">
                                                Data
                                              </th>
                                              <th className="text-left text-gray-500 px-2 py-1">
                                                Tipo
                                          ...
                                          <tbody>
                                            {evento.dati_presenze_cedente_json
                                              ...
                                              .map((p: any, pidx: number) => (
                                                ...
                                                >
                                                  <td className="text-gray-300 px-2 py-1">
                                                    {formatDate(
                                                      p.data_presenza || p.date
                                                    )}
                                                  </td>
                                                  <td className="text-gray-300 px-2 py-1">
                                                    {p.tipo_presenza ||
      ...
      {chains.length > 0 && (
        <div className="flex gap-3 text-sm text-gray-500 flex-wrap">
          ...
          <span>
            Subingressi:{" "}
            {eventi.filter(e => e.tipo_evento === "SUBINGRESSO").length}
          </span>
          <span>|</span>
          <span>
            Rinnovi: {eventi.filter(e => e.tipo_evento === "RINNOVO").length}
          </span>
        </div>
      )}
    </div>
