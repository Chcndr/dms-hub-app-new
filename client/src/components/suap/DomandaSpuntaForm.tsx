...
export default function DomandaSpuntaForm({
  onCancel,
  onSubmit,
  initialData,
  domandaId,
  mode = "create",
}: DomandaSpuntaFormProps) {
        ...
        try {
          ...
          if (json.success && json.data) {
            ...
            setFormData(prev => ({
              ...
              data_richiesta: dom.data_richiesta
                ? new Date(dom.data_richiesta).toISOString().split("T")[0]
                : "",
              note: dom.note || "",
  ...
  const selectImpresa = async (impresa: Impresa) => {
    ...
    try {
      ...
      if (authJson.success && authJson.data) {
        ...
        const validAuth = authJson.data.filter(
          (a: Autorizzazione) =>
        ...
        if (validAuth.length === 0) {
          toast.warning("Nessuna autorizzazione itinerante attiva", {
            description:
              "Per partecipare alla spunta serve un'autorizzazione tipo B",
          });
        }
      }
    } catch (err) {
    ...
    try {
      ...
      if (json.success && json.data && json.data.length > 0) {
        ...
        const antimafiaQualifica = qualifiche.find((q: any) => {
          ...
          const scadenza = new Date(q.data_scadenza);
          return (
            tipo.includes("ANTIMAFIA") && stato !== "SCADUTO" && scadenza > oggi
          );
        });
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.impresa_id || !formData.mercato_id) {
      toast.error("Compila i campi obbligatori", {
        description: "Impresa e Mercato sono richiesti",
      });
      return;
    }

    if (!formData.dichiarazione_requisiti || !formData.dichiarazione_durc) {
      toast.error("Conferma le dichiarazioni", {
        description: "Devi confermare i requisiti e il DURC",
      });
      return;
    }
      ...
      const response = await authenticatedFetch(
        `${API_URL}/api/domande-spunta`,
        {
          ...
          body: JSON.stringify({
            ...
            autorizzazione_id: formData.autorizzazione_id
              ? parseInt(formData.autorizzazione_id)
              : null,
      ...
      <CardHeader className="border-b border-[#1e293b]">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-[#e8fbff]">
            {isViewMode
              ? "Dettaglio Domanda Spunta"
              : isEditMode
                ? "Modifica Domanda Spunta"
                : "Domanda di Partecipazione alla Spunta"}
          </CardTitle>
        </div>
        <CardDescription className="text-gray-400">
          Richiesta per partecipare all'assegnazione giornaliera dei posteggi
          liberi
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6 pt-6">
        ...
        <Alert className="bg-[#0a2540] border-green-600/50">
          ...
          <AlertDescription className="text-[#e8fbff]">
            Inviando questa domanda, verrà creato automaticamente un{" "}
            <strong>Wallet Spunta</strong> per l'impresa, che permetterà di
            pagare i canoni giornalieri per i posteggi assegnati.
          </AlertDescription>
        </Alert>
        ...
        <div className="space-y-4 border p-4 rounded-lg border-[#1e293b]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={formData.settore_richiesto}
                onValueChange={val =>
                  setFormData({ ...formData, settore_richiesto: val })
                }
              >
                <SelectTrigger className="bg-[#020817] border-[#1e293b] text-[#e8fbff]">
                  <SelectValue />
        ...
        <div className="space-y-4 border p-4 rounded-lg border-[#1e293b]">
          <h3 className="text-sm font-semibold text-[#e8fbff]">
            Autorizzazione Commercio (Tipo B - Itinerante)
          </h3>
        ...
        <div className="space-y-4 border p-4 rounded-lg border-[#1e293b]">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-[#e8fbff]">
              Presenze per Graduatoria Spunta
            </h3>
            <span className="text-xs text-gray-400">
              (Inserire manualmente o auto-compilato da graduatoria)
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    numero_presenze: parseInt(e.target.value) || 0,
                  })
                }
                placeholder="0"
                className="bg-[#020817] border-[#1e293b] text-[#e8fbff]"
              ...
              <p className="text-xs text-gray-400">
                Numero di presenze maturate nel mercato selezionato
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-gray-400">
                Data della prima presenza registrata nel mercato
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-start space-x-3">
              <Checkbox
                ...
                onCheckedChange={checked =>
                  setFormData({
                    ...formData,
                    dichiarazione_requisiti: checked as boolean,
                  })
                }
              ...
              <Label
                htmlFor="requisiti"
                className="text-[#e8fbff] text-sm leading-relaxed"
              >
                Dichiaro di essere in possesso dei requisiti morali e
                professionali previsti dall'art. 5 del D.Lgs. 114/1998
              </Label>
            </div>
            ...
            <div className="flex items-start space-x-3">
              <Checkbox
                ...
                onCheckedChange={checked =>
                  setFormData({
                    ...formData,
                    dichiarazione_durc: checked as boolean,
                  })
                }
                className="border-[#1e293b] data-[state=checked]:bg-green-600"
              />
              <Label
                ...
                className="text-[#e8fbff] text-sm leading-relaxed"
              >
                Dichiaro di essere in regola con gli obblighi contributivi (DURC
                regolare)
              </Label>
            </div>
            ...
            <div className="flex items-start space-x-3">
              <Checkbox
                ...
                onCheckedChange={checked =>
                  setFormData({
                    ...formData,
                    dichiarazione_antimafia: checked as boolean,
                  })
                }
                className="border-[#1e293b] data-[state=checked]:bg-green-600"
              />
              <Label
                ...
                className="text-[#e8fbff] text-sm leading-relaxed"
              >
                Dichiaro l'assenza di cause di divieto, decadenza o sospensione
                di cui all'art. 67 del D.Lgs. 159/2011 (Antimafia)
              </Label>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 border p-4 rounded-lg border-[#1e293b]">
          ...
          <p className="text-xs text-[#e8fbff]/50">
            Due marche da bollo da 16,00€: una per la domanda di autorizzazione
            e una per il rilascio della concessione (D.M. 10/11/2011).
          </p>

          {/* Bollo 1 */}
          <div className="p-3 bg-[#020817] rounded-lg border border-[#f59e0b]/20">
            <h4 className="text-xs font-semibold text-[#f59e0b] mb-2">
              Marca da Bollo n. 1 (Domanda - 16,00€)
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-[#e8fbff] text-xs">
          ...
          <div className="p-3 bg-[#020817] rounded-lg border border-[#f59e0b]/20">
            <h4 className="text-xs font-semibold text-[#f59e0b] mb-2">
              Marca da Bollo n. 2 (Concessione - 16,00€)
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-[#e8fbff] text-xs">
          ...
          <div className="p-3 bg-[#020817] rounded-lg border border-[#f59e0b]/30">
            <div className="flex items-start space-x-3">
              ...
              />
              <Label
                htmlFor="bollo_dichiarazione_spunta"
                className="text-[#e8fbff] text-xs leading-relaxed cursor-pointer"
              >
                <strong>Dichiarazione sostitutiva di atto di notorietà</strong>{" "}
                (ai sensi dell'art. 47 del D.P.R. 28 dicembre 2000, n. 445) —
                Il/La sottoscritto/a dichiara di aver assolto l'imposta di bollo
                ai sensi del D.M. 10 novembre 2011, di aver annullato le marche
                da bollo sopra indicate apponendovi la data e la propria firma,
                e che le stesse non sono state e non saranno utilizzate per
                qualsiasi altro adempimento. Il/La sottoscritto/a è consapevole
                che, ai sensi dell'art. 76 del D.P.R. 445/2000, le dichiarazioni
                mendaci sono punite ai sensi del codice penale e delle leggi
                speciali in materia.
              </Label>
            </div>
          </div>
