...
function APILogsTab() {
  ...
  useEffect(() => {
    const loadLogs = async () => {
      try {
        ...
        const convertedLogs: APILog[] = data.logs.map(
          (log: any, index: number) => ({
            id: index + 1,
            timestamp: new Date(log.timestamp),
            method:
              (log.message?.match(/(GET|POST|PUT|DELETE|PATCH)/)?.[1] as any) ||
              log.method ||
              "GET",
            endpoint: log.source,
  ...
  const filteredLogs = logs.filter(log => {
    if (filterEndpoint !== "all" && !log.endpoint.includes(filterEndpoint))
      return false;
    if (filterStatus !== "all") {
      if (
        filterStatus === "2xx" &&
            ...
            {filteredLogs.map(log => (
              ...
              >
                ...
                <div className="mt-3 pt-3 border-t border-[#14b8a6]/20 space-y-2">
                  {log.requestBody && (
                    <div>
                      <p className="text-xs text-[#e8fbff]/70 mb-1">
                        Request Body:
                      </p>
              ...
              >
                ...
                {log.error && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded p-2 text-xs text-red-300">
                    ...
                    {log.retryCount > 0 && (
                      <span className="ml-2">(Retry: {log.retryCount})</span>
                    )}
                  </div>
                )}
              </div>
...
function SystemStatusTab() {
      ...
      try {
        ...
        const convertedStatus: SystemStatus = {
          backend: {
            ...
            status:
              data.status.processes.find((p: any) => p.name === "mihub-backend")
                ?.status === "online"
                ? "up"
                : "down",
            uptime: (() => {
              const proc = data.status.processes.find(
                (p: any) => p.name === "mihub-backend"
              );
              if (!proc) return 0;
              const restarts =
  ...
  if (loading || !status) {
    return (
      <div className="text-center text-[#e8fbff]/60 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[#e8fbff]">
                {status.backend.lastCheck.toLocaleTimeString("it-IT")}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="bg-[#1a2332] border-[#14b8a6]/30">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-sm text-[#e8fbff]/70 mb-1">
                Connessioni Attive
              </p>
              <p className="text-lg font-bold text-[#14b8a6]">
                {status.database.activeConnections}/
            ...
            <div>
              ...
              <p className="text-sm text-[#e8fbff]">
                {status.database.lastQuery.toLocaleTimeString("it-IT")}
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.entries(status.integrations).map(([name, state]) => (
              <div
                key={name}
                className="bg-[#0a1628] p-4 rounded-lg border border-[#14b8a6]/20"
              >
