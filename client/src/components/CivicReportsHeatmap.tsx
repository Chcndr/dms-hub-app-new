...
function MapCenterUpdater({
  ...
  selectedReport: { id: number; lat: number; lng: number; type: string } | null;
}) {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    // Se c'è una segnalazione selezionata, centra su quella
    if (selectedReport && selectedReport.lat && selectedReport.lng) {
      map.flyTo([selectedReport.lat, selectedReport.lng], 18, { duration: 1 });
      return;
    }
    ...
    if (validReports.length > 0) {
      const avgLat =
        validReports.reduce(
          (sum, r) =>
            sum + (typeof r.lat === "string" ? parseFloat(r.lat) : r.lat),
          0
        ) / validReports.length;
      const avgLng =
        validReports.reduce(
          (sum, r) =>
            sum + (typeof r.lng === "string" ? parseFloat(r.lng) : r.lng),
          0
        ) / validReports.length;
  ...
  }, [map, reports, comuneId, selectedReport]);
...
export default function CivicReportsHeatmap() {
  ...
  const markersWithOffset: Array<
    CivicReport & { offsetLat: number; offsetLng: number }
  > = [];
  ...
  const getInitialCenter = (): [number, number] => {
    if (currentComuneId && COMUNI_COORDS[currentComuneId]) {
      return [
        COMUNI_COORDS[currentComuneId].lat,
        COMUNI_COORDS[currentComuneId].lng,
      ];
    }
        ...
        <div className="flex items-center gap-2">
          ...
          {validReports.length > 0 && (
            <span className="text-xs text-emerald-400">
              ● {validReports.length} segnalazioni
            </span>
          )}
        </div>
      </div>
