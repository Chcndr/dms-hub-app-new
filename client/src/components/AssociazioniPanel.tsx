...
export default function AssociazioniPanel() {
  ...
  const [activeTab, setActiveTab] = useState<
    "anagrafica" | "contatti" | "fatturazione" | "permessi"
  >("anagrafica");
  ...
  const RUOLI_ASSOCIAZIONE = [
    {
      value: "admin",
      label: "Admin Associazione",
      description: "Accesso completo a tutte le funzionalità",
    },
    {
      value: "presidente",
      label: "Presidente",
      description: "Visione completa e approvazione pratiche",
    },
    {
      value: "referente_scia",
      label: "Referente SCIA",
      description: "Gestione pratiche SCIA e Domande Spunta",
    },
    {
      value: "operatore",
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Sei sicuro di voler eliminare questa associazione e tutti i dati correlati?"
      )
    )
      return;
    try {
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/associazioni/${id}`,
        { method: "DELETE" }
      );
      const data = await res.json();
      if (data.success) {
  ...
  const openEdit = (a: Associazione) => {
    ...
    setForm({
      ...
      logo_url: a.logo_url || "",
      presidente_nome: a.presidente_nome || "",
      presidente_cognome: a.presidente_cognome || "",
      ...
      referente_telefono: a.referente_telefono || "",
      delegato_nome: a.delegato_nome || "",
      delegato_cognome: a.delegato_cognome || "",
      delegato_codice_fiscale: a.delegato_codice_fiscale || "",
      delegato_data_nascita: a.delegato_data_nascita
        ? a.delegato_data_nascita.split("T")[0]
        : "",
      ...
      num_tesserati: String(a.num_tesserati || ""),
      stato: a.stato || "attivo",
      note: a.note || "",
    });
    setShowForm(true);
  };
  ...
  const handleCreateContratto = async () => {
    ...
    try {
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/associazioni/${selectedAssociazione.id}/contratti`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            percentuale_pratica:
              parseFloat(contrattoForm.percentuale_pratica) || 0,
          }),
        }
      );
      const data = await res.json();
  ...
  const handleDeleteContratto = async (contrattoId: number) => {
    ...
    try {
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/associazioni/contratti/${contrattoId}`,
        { method: "DELETE" }
      );
      const data = await res.json();
  ...
  const handleCreateFattura = async () => {
    ...
    try {
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/associazioni/${selectedAssociazione.id}/fatture`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleUpdateFatturaStato = async (
    ...
    try {
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/associazioni/fatture/${fatturaId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleAssegnaUtente = async () => {
    ...
    try {
      ...
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/associazioni/${selectedAssociazione.id}/utenti`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleUpdateRuolo = async (
    ...
    try {
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/associazioni/utenti/${assegnazioneId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleRimuoviUtente = async (assegnazioneId: number) => {
    if (
      !confirm("Sei sicuro di voler rimuovere questo utente dall'associazione?")
    )
      return;
    try {
      const res = await authenticatedFetch(
    ...
    try {
      ...
      const adminUser =
        data.success && data.data.length > 0
          ? data.data.find(
              (u: UtenteAssociazione) => u.ruolo === "admin" && u.attivo
            ) || data.data.find((u: UtenteAssociazione) => u.attivo)
          : null;
      ...
      const userEmail = adminUser
        ...
        : encodeURIComponent(
            `admin@assoc_${assoc.sigla?.toLowerCase() || assoc.id}.miohub.it`
          );
      ...
      if (
        confirm(
          `Vuoi accedere come Admin dell'Associazione ${assoc.nome}?\n\nVerrai reindirizzato alla dashboard dell'associazione.`
        )
      ) {
  ...
  const totalTesserati = associazioni.reduce(
    (acc, a) => acc + (Number(a.num_tesserati) || 0),
    0
  );
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
          ...
          <div className="text-2xl font-bold text-amber-400">
            {associazioni.length > 0
              ? (totalTesserati / associazioni.length).toFixed(0)
              : "0"}
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-6">
        {/* Lista */}
        <div
          className={`bg-gray-800/50 rounded-xl border border-gray-700 p-4 ${selectedAssociazione ? "w-80 flex-shrink-0" : "w-full"}`}
        >
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-8 text-gray-400">
              ...
              <p className="text-sm">
                Clicca "Nuova Associazione" per iniziare
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-gray-500">
                          {TIPI_ASSOCIAZIONE.find(t => t.value === assoc.tipo)
                            ?.label || assoc.tipo}
                        </span>
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={e => {
                          e.stopPropagation();
                          handleAccediComeAssociazione(assoc);
                        }}
          ...
          <div className="flex-1 bg-gray-800/50 rounded-xl border border-gray-700 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() =>
                    handleAccediComeAssociazione(selectedAssociazione)
                  }
                ...
                <span
                  className={`text-xs px-2 py-1 rounded ${
                    selectedAssociazione.stato === "attivo"
                      ? "bg-emerald-600/30 text-emerald-300"
                      : selectedAssociazione.stato === "sospeso"
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                  ...
                  {selectedAssociazione.citta && (
                    <div className="flex items-center gap-2 text-gray-300">
                      ...
                      {selectedAssociazione.citta}{" "}
                      {selectedAssociazione.provincia &&
                        `(${selectedAssociazione.provincia})`}{" "}
                      - {selectedAssociazione.regione}
                    </div>
                  )}
                  ...
                  {selectedAssociazione.sito_web && (
                    <div className="flex items-center gap-2 text-cyan-400">
                      ...
                      <a
                        href={selectedAssociazione.sito_web}
                        target="_blank"
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4 p-3 bg-gray-700/30 rounded-lg text-xs">
                  ...
                  <div>
                    ...
                    <span className="text-cyan-300 ml-1">
                      {TIPI_ASSOCIAZIONE.find(
                        t => t.value === selectedAssociazione.tipo
                      )?.label || selectedAssociazione.tipo}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">Tesserati:</span>
                    <span className="text-emerald-300 ml-1">
                      {selectedAssociazione.num_tesserati || 0}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <span className="text-white ml-2">
                        {selectedAssociazione.presidente_nome || "-"}{" "}
                        {selectedAssociazione.presidente_cognome || ""}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">Email:</span>
                      <span className="text-cyan-400 ml-2">
                        {selectedAssociazione.presidente_email || "-"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-gray-700/30 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="text-gray-500">Nome:</span>
                      <span className="text-white ml-2">
                        {selectedAssociazione.referente_nome || "-"}{" "}
                        {selectedAssociazione.referente_cognome || ""}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">Email:</span>
                      <span className="text-cyan-400 ml-2">
                        {selectedAssociazione.referente_email || "-"}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">Telefono:</span>
                      <span className="text-white ml-2">
                        {selectedAssociazione.referente_telefono || "-"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-gray-700/30 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="text-gray-500">Nome:</span>
                      <span className="text-white ml-2">
                        {selectedAssociazione.delegato_nome || "-"}{" "}
                        {selectedAssociazione.delegato_cognome || ""}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">Codice Fiscale:</span>
                      <span className="text-white ml-2">
                        {selectedAssociazione.delegato_codice_fiscale || "-"}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">Data Nascita:</span>
                      <span className="text-white ml-2">
                        {selectedAssociazione.delegato_data_nascita
                          ? new Date(
                              selectedAssociazione.delegato_data_nascita
                            ).toLocaleDateString("it-IT")
                          : "-"}
                    ...
                    <div>
                      ...
                      <span className="text-white ml-2">
                        {selectedAssociazione.delegato_luogo_nascita || "-"}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">Qualifica:</span>
                      <span className="text-white ml-2">
                        {selectedAssociazione.delegato_qualifica || "-"}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">PEC:</span>
                      <span className="text-cyan-400 ml-2">
                        {selectedAssociazione.delegato_pec || "-"}
                      </span>
                    </div>
                    <div className="col-span-2">
                      <span className="text-gray-500">Residenza:</span>
                      <span className="text-white ml-2">
                        {selectedAssociazione.delegato_residenza_via || "-"},{" "}
                        {selectedAssociazione.delegato_residenza_comune || ""}{" "}
                        {selectedAssociazione.delegato_residenza_cap || ""}
                      </span>
                    </div>
                  </div>
                </div>
                            ...
                            >
                              <div>
                                ...
                                <div className="text-xs text-gray-400">
                                  ...
                                  {c.percentuale_pratica > 0 && (
                                    <span className="text-amber-400 ml-2">
                                      Fee: {c.percentuale_pratica}%
                                    </span>
                                  )}
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-emerald-400">
                                  {Number(c.importo_annuale).toLocaleString(
                                    "it-IT",
                                ...
                                <button
                                  onClick={() => handleDeleteContratto(c.id)}
                                  className="p-1 text-gray-400 hover:text-red-400"
                                >
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-white">
                                  {Number(f.totale).toLocaleString("it-IT", {
                                    style: "currency",
                                    currency: "EUR",
                                  })}
                                </span>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      {RUOLI_ASSOCIAZIONE.map(r => (
                        <div
                          key={r.value}
                          className="p-2 bg-gray-700/30 rounded text-xs"
                        >
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2">
                                <select
                                  value={u.ruolo}
                                  onChange={e =>
                                    handleUpdateRuolo(u.id, e.target.value)
                                  }
                                ...
                                <button
                                  onClick={() => handleRimuoviUtente(u.id)}
                                  className="p-1 text-gray-400 hover:text-red-400"
                                >
                                  <Trash2 className="w-3 h-3" />
                                </button>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input
                  ...
                  placeholder="es. Confcommercio Grosseto"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setForm({ ...form, presidente_nome: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  value={form.presidente_cognome}
                  onChange={e =>
                    setForm({ ...form, presidente_cognome: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div className="col-span-2">
                ...
                <input
                  value={form.presidente_email}
                  onChange={e =>
                    setForm({ ...form, presidente_email: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setForm({ ...form, referente_nome: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                <label className="text-xs text-gray-400">Cognome</label>
                <input
                  value={form.referente_cognome}
                  onChange={e =>
                    setForm({ ...form, referente_cognome: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setForm({ ...form, referente_email: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setForm({ ...form, referente_telefono: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              ...
              <div>
                ...
                <input
                  value={form.delegato_nome}
                  onChange={e =>
                    setForm({ ...form, delegato_nome: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  value={form.delegato_cognome}
                  onChange={e =>
                    setForm({ ...form, delegato_cognome: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  value={form.delegato_codice_fiscale}
                  onChange={e =>
                    setForm({
                      ...form,
                      delegato_codice_fiscale: e.target.value,
                    })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              ...
              <div>
                ...
                <input
                  type="date"
                  value={form.delegato_data_nascita}
                  onChange={e =>
                    setForm({ ...form, delegato_data_nascita: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  value={form.delegato_luogo_nascita}
                  onChange={e =>
                    setForm({ ...form, delegato_luogo_nascita: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  value={form.delegato_qualifica}
                  onChange={e =>
                    setForm({ ...form, delegato_qualifica: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  value={form.delegato_residenza_via}
                  onChange={e =>
                    setForm({ ...form, delegato_residenza_via: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  value={form.delegato_residenza_comune}
                  onChange={e =>
                    setForm({
                      ...form,
                      delegato_residenza_comune: e.target.value,
                    })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              ...
              <div>
                <label className="text-xs text-gray-400">CAP</label>
                <input
                  value={form.delegato_residenza_cap}
                  onChange={e =>
                    setForm({ ...form, delegato_residenza_cap: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              <div>
                ...
                <input
                  value={form.delegato_pec}
                  onChange={e =>
                    setForm({ ...form, delegato_pec: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  type="number"
                  value={form.num_tesserati}
                  onChange={e =>
                    setForm({ ...form, num_tesserati: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    step="0.5"
                    value={contrattoForm.percentuale_pratica}
                    onChange={e =>
                      setContrattoForm({
                        ...contrattoForm,
                        percentuale_pratica: e.target.value,
                      })
                    }
                    className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                    placeholder="es. 5"
