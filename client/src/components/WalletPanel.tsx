...
export default function WalletPanel() {
  const [subTab, setSubTab] = useState<
    ...
    | "sanzioni"
    | "notifiche"
  >("wallet"); // v3.53.0: aggiunto sanzioni
  ...
  const [canoneFilters, setCanoneFilters] = useState({
    mercato_id: "all",
    tipo_operatore: "all",
    impresa_search: "",
    stato: "all",
  ...
  const [
    showPagamentoStraordinarioDialog,
    setShowPagamentoStraordinarioDialog,
  ] = useState(false);
  const [canoneAnno, setCanoneAnno] = useState(
    new Date().getFullYear().toString()
  );
  const [canoneDataPrimaRata, setCanoneDataPrimaRata] = useState(
  ...
  const [straordinarioDataScadenza, setStraordinarioDataScadenza] =
    useState("");
  const [straordinarioImpresaId, setStraordinarioImpresaId] = useState("");
  const [straordinarioImpresaSearch, setStraordinarioImpresaSearch] =
    useState("");
  const [straordinarioPosteggio, setStraordinarioPosteggio] = useState("");
  const [straordinarioImpreseSuggestions, setStraordinarioImpreseSuggestions] =
    useState<any[]>([]);
  const [straordinarioPosteggiList, setStraordinarioPosteggiList] = useState<{
    posteggi_impresa: any[];
    altri_posteggi: any[];
  }>({ posteggi_impresa: [], altri_posteggi: [] });
  ...
  const [annualFeeData, setAnnualFeeData] =
    useState<AnnualFeeCalculation | null>(null);
  ...
  const [mercatiList, setMercatiList] = useState<
  ...
  const [impreseTipoOperatore, setImpreseTipoOperatore] =
    useState<string>("all"); // v3.58.0: filtro tipo operatore
  ...
  const [sanzioniFilters, setSanzioniFilters] = useState({
    stato: "all",
    impresa_search: "",
  });
  const [sanzioniTotali, setSanzioniTotali] = useState({
    non_pagato: 0,
    pagato: 0,
    totale: 0,
  });
  const [showRegistraPagamentoDialog, setShowRegistraPagamentoDialog] =
    useState(false);
  ...
  const [showImpostazioniMoraDialog, setShowImpostazioniMoraDialog] =
    useState(false);
    ...
    try {
      ...
      if (data.success) {
        ...
        const grouped = data.data.reduce(
          (acc: Record<number, CompanyWallets>, row: any) => {
            ...
            let typeNormalized = row.type
              ? row.type.toUpperCase().trim()
              : "SPUNTA";
  ...
  const ensureWalletTablesExist = async () => {
    try {
      await authenticatedFetch(`${WALLET_API_BASE}/api/wallets/init`, {
        method: "POST",
    ...
    try {
      ...
      if (data.success) {
        setImpostazioniMora({
          ...
          tasso_interesse_giornaliero: parseFloat(
            data.impostazioni.tasso_interesse_giornaliero
          ),
  ...
  const handleSaveImpostazioniMora = async () => {
    ...
    try {
      ...
      const response = await authenticatedFetch(
        `${API_URL}/api/canone-unico/impostazioni-mora`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        ...
        if (impostazioniMora.mora_abilitata) {
          await authenticatedFetch(
            `${API_URL}/api/canone-unico/aggiorna-mora`,
            { method: "POST" }
          );
          fetchCanoneScadenze();
        }
  ...
  const fetchCanoneScadenze = async () => {
    ...
    try {
      ...
      if (canoneFilters.mercato_id && canoneFilters.mercato_id !== "all")
        params.append("mercato_id", canoneFilters.mercato_id);
      if (
        canoneFilters.tipo_operatore &&
        canoneFilters.tipo_operatore !== "all"
      )
        params.append("tipo_operatore", canoneFilters.tipo_operatore);
      if (canoneFilters.impresa_search)
        params.append("impresa_search", canoneFilters.impresa_search);
      if (canoneFilters.stato && canoneFilters.stato !== "all")
  ...
  const handleGeneraCanoneAnnuo = async () => {
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(
        `${API_URL}/api/canone-unico/genera-canone-annuo`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            mercato_id:
              canoneFilters.mercato_id !== "all"
                ? parseInt(canoneFilters.mercato_id)
                : null,
          }),
        }
      );
      ...
      if (data.success) {
        const mercatoNome =
          canoneFilters.mercato_id !== "all"
            ? mercatiList.find(
                (m: any) => m.id.toString() === canoneFilters.mercato_id
              )?.name || "mercato selezionato"
            : "TUTTI i mercati";
        alert(
          `Generate ${data.scadenze_create} scadenze (${data.numero_rate} rate) per l'anno ${canoneAnno}\nMercato: ${mercatoNome}`
        );
  ...
  const handleDeleteScadenza = async (scadenzaId: number) => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(
        `${API_URL}/api/canone-unico/scadenza/${scadenzaId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleDeleteScadenzeAnno = async () => {
    const anno = prompt(
      "Inserisci l'anno delle scadenze da eliminare (es. 2026):"
    );
    if (!anno) return;
    if (
      !confirm(
        `Sei sicuro di voler eliminare TUTTE le scadenze dell'anno ${anno}?`
      )
    )
      return;
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(
        `${API_URL}/api/canone-unico/scadenze/${anno}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (data.success) {
        alert(
          `Eliminate ${data.scadenze_eliminate} scadenze per l'anno ${anno}`
        );
        fetchCanoneScadenze();
      } else {
        alert("Errore: " + data.error);
  ...
  const handleGeneraPagamentoStraordinario = async () => {
    if (
      !straordinarioDescrizione ||
      !straordinarioImporto ||
      !straordinarioDataScadenza
    ) {
      alert(
        "Compila tutti i campi obbligatori: Descrizione, Importo, Data Scadenza"
      );
      return;
    }
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(
        `${API_URL}/api/canone-unico/genera-pagamento-straordinario`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            mercato_id:
              straordinarioMercatoId !== "all"
                ? parseInt(straordinarioMercatoId)
                : null,
            impresa_id: straordinarioImpresaId
              ? parseInt(straordinarioImpresaId)
              : null,
      ...
      if (data.success) {
        alert(
          `Generati ${data.scadenze_create} avvisi di pagamento straordinario per ${data.imprese_coinvolte} imprese`
        );
  ...
  const fetchImpreseConcessioni = async (
    marketId: string,
  ...
  useEffect(() => {
    if (straordinarioMercatoId && straordinarioMercatoId !== "all") {
      loadPosteggiMercato(
        ...
        straordinarioImpresaId || undefined
      );
    } else {
      setStraordinarioPosteggiList({
        posteggi_impresa: [],
  ...
  const fetchSanzioni = async () => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const params = new URLSearchParams();
      if (sanzioniFilters.stato !== "all")
        params.append("stato", sanzioniFilters.stato);
      if (sanzioniFilters.impresa_search)
        params.append("impresa_search", sanzioniFilters.impresa_search);
      ...
      if (data.success) {
        ...
        setSanzioniTotali(
          data.totali || { non_pagato: 0, pagato: 0, totale: 0 }
        );
      }
    } catch (error) {
  ...
  const handleOpenDeposit = async (wallet: WalletItem, companyName: string) => {
    ...
    try {
      ...
      if (
        scadenzeData.success &&
        scadenzeData.semaforo &&
        scadenzeData.semaforo.length > 0
      ) {
        ...
        const rateNonPagate = scadenzeData.semaforo.filter(
          (r: any) => r.stato !== "PAGATO"
        );
        ...
        const res = await authenticatedFetch(
          `${API_URL}/api/wallets/calculate-annual-fee`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    try {
      ...
      if (selectedWallet.type !== "SPUNTA") {
        if (walletScadenze.length > 0) {
          ...
          const primaRataDaPagare = walletScadenze.find(
            s => s.stato !== "PAGATO"
          );
        ...
        if (walletScadenze.length > 0) {
          const primaScadenzaDaPagare = walletScadenze.find(
            s => s.stato !== "PAGATO"
          );
          if (primaScadenzaDaPagare) {
            scadenza_id = primaScadenzaDaPagare.id;
            description =
              primaScadenzaDaPagare.causale || `Pagamento Straordinario`;
          } else {
      ...
      const res = await authenticatedFetch(`${API_URL}/api/wallets/deposit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (data.success) {
        alert(
          mode === "AVVISO"
            ? "Avviso PagoPA generato con successo!"
  ...
  const handleDeleteWallet = async () => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const res = await authenticatedFetch(
        `${API_URL}/api/wallets/${walletToDelete.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (data.success) {
        ...
        alert(
          "Errore: " + (data.error || data.message || "Eliminazione fallita")
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleAzzeraWallet = async (walletId: number) => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const res = await authenticatedFetch(
        `${API_URL}/api/canone-unico/wallet/${walletId}/azzera`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        }
      ...
      if (data.success) {
        alert(
          `Wallet #${walletId} azzerato! Saldo precedente: €${data.saldo_precedente}`
        );
        fetchWallets();
        fetchCanoneScadenze();
      } else {
  ...
  const handleAzzeraTuttiWallet = async () => {
    if (
      !confirm(
        "ATTENZIONE: Vuoi azzerare il saldo di TUTTI i wallet? Questa operazione è per test."
      )
    )
      return;
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const res = await authenticatedFetch(
        `${API_URL}/api/canone-unico/wallets/azzera-tutti`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        }
      ...
      if (data.success) {
        alert(
          `${data.wallets_azzerati} wallet azzerati! Totale: €${data.totale_azzerato}`
        );
        fetchWallets();
        fetchCanoneScadenze();
      } else {
  ...
  return (
    <div className="space-y-6 p-6 bg-[#0f172a] min-h-screen text-slate-100 border border-green-500/30 rounded-lg">
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Gestione borsellini digitali: Spunta (ricaricabile) e Concessioni
            (canone annuo)
          </p>
        </div>
            ...
            <div className="grid gap-6">
              {filteredCompanies.map(company => (
                <Card
                  ...
                  className="bg-[#1e293b] border-slate-700 overflow-hidden"
                >
                      ...
                      <div className="text-right flex flex-col items-end gap-1">
                        ...
                        >
                          WALLET:{" "}
                          {
                            ...
                            company.spunta_wallets.filter(w => w.market_name)
                              .length + company.concession_wallets.length
                          }
                        </Badge>
                        <div className="text-sm font-bold text-white">
                          ...
                          {(
                            ...
                            company.concession_wallets.reduce(
                              (acc, w) => acc + w.balance,
                              0
                            )
                          ).toFixed(2)}
                  ...
                  <CardContent className="pt-4 space-y-4">
                    ...
                    <Collapsible defaultOpen={false}>
                      <CollapsibleTrigger asChild>
                        <Button
                          variant="ghost"
                          className="w-full flex justify-between items-center text-slate-300 hover:text-white hover:bg-slate-800 mb-2"
                        >
                      ...
                      <CollapsibleContent className="space-y-3">
                        {company.spunta_wallets.length > 0 ? (
                          company.spunta_wallets.map(wallet => (
                            <div
                              key={wallet.id}
                              className="flex flex-col md:flex-row justify-between items-center p-4 bg-[#0f172a] rounded-lg border border-slate-700 gap-4"
                            >
                              ...
                              <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                                <div className="text-right">
                                  ...
                                  <p
                                    className={`text-xl font-bold ${wallet.balance <= 0 ? "text-red-500" : "text-green-500"}`}
                                  >
                                    € {wallet.balance.toFixed(2)}
                                  </p>
                                  <p
                                    className={`text-xs ${wallet.balance <= 0 ? "text-red-400" : "text-green-400"}`}
                                  >
                                    {wallet.balance <= 0
                                      ? "Da Ricaricare"
                                      : "In Regola"}
                    ...
                    <Collapsible>
                      <CollapsibleTrigger asChild>
                        <Button
                          variant="ghost"
                          className="w-full flex justify-between items-center text-slate-300 hover:text-white hover:bg-slate-800"
                        >
                      ...
                      <CollapsibleContent className="pt-4 space-y-3">
                        {company.concession_wallets.length > 0 ? (
                            ...
                            >
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <span className="text-slate-300 font-medium">
                                    Posteggio {wallet.stall_number}
                                  </span>
                                </div>
                                <div className="text-sm text-slate-500 flex gap-4">
                                  <span>
                                    Area:{" "}
                                    {isNaN(wallet.stall_area || NaN)
                                      ? "-"
                                      : wallet.stall_area}{" "}
                                    mq
                                  </span>
                                  <span>
                                    Tariffa:{" "}
                                    {isNaN(wallet.cost_per_sqm || NaN)
                                      ? "-"
                                      : `€ ${wallet.cost_per_sqm}/mq`}
                                  </span>
                                  <span>
                                    Giorni:{" "}
                                    {isNaN(wallet.annual_market_days || NaN)
                                      ? "-"
                                      : `${wallet.annual_market_days}/anno`}
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="text-right min-w-[120px]">
                                ...
                                <p
                                  className={`text-lg font-bold ${wallet.balance > 0 ? "text-green-500" : "text-red-500"}`}
                                >
                                  € {wallet.balance.toFixed(2)}
                                </p>
                                <p
                                  className={`text-xs ${wallet.balance > 0 ? "text-green-400" : "text-red-400"}`}
                                >
                                  {wallet.balance > 0
                                    ? "In Regola"
                                    : "Da Pagare"}
                          ...
                          <p className="text-center text-slate-500 py-4">
                            Nessuna concessione attiva per questa impresa.
                          </p>
                        )}
                      </CollapsibleContent>
                    </Collapsible>
      ...
      {subTab === "pagopa" && (
        <div className="space-y-6">
          ...
          <Card className="bg-[#1e293b] border-slate-700">
            ...
            <CardContent>
              {sanzioniList.filter(s => s.payment_status === "PAGATO")
                .length === 0 ? (
                ...
                <div className="rounded-md border border-slate-700 overflow-hidden">
                  <table className="w-full text-sm text-left text-slate-300">
                    ...
                    <tbody className="divide-y divide-slate-700">
                      {sanzioniList
                        .filter(s => s.payment_status === "PAGATO")
                        .map((sanzione, idx) => {
                          const importoEffettivo =
                            sanzione.importo_effettivo_pagato
                              ? parseFloat(sanzione.importo_effettivo_pagato)
                              : sanzione.pagato_in_ridotto
                                ? parseFloat(sanzione.importo_ridotto)
                                : parseFloat(sanzione.amount);
                          const haSconto =
                            sanzione.pagato_in_ridotto ||
                            (sanzione.reduced_amount &&
                              parseFloat(sanzione.reduced_amount) <
                                parseFloat(sanzione.amount));
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-4 py-3">
                                ...
                                {sanzione.partita_iva && (
                                  <span className="block text-xs text-slate-500">
                                    P.IVA: {sanzione.partita_iva}
                                  </span>
                                )}
                              </td>
                              <td className="px-4 py-3 text-xs">
                                {sanzione.infraction_description ||
                                  sanzione.infraction_code}
                              </td>
        ...
        <div className="space-y-6">
          <Card className="bg-[#1e293b] border-slate-700">
            <CardHeader>
              ...
              <p className="text-sm text-slate-400 mt-1">
                Cronologia creazione, eliminazione e trasferimenti wallet con
                motivi e saldi residui
              </p>
            </CardHeader>
            <CardContent>
              {isLoadingHistory ? (
                ...
                <div className="text-center py-12 text-slate-500">
                  ...
                  <p className="text-sm mt-2">
                    Gli eventi verranno registrati automaticamente durante le
                    operazioni sui wallet.
                  </p>
                </div>
              ) : (
                <div className="rounded-md border border-slate-700 overflow-hidden">
                          ...
                          <td className="px-4 py-3 text-slate-400 text-xs">
                            {event.mercato_nome && (
                              <span className="block">
                                Mercato: {event.mercato_nome}
                              </span>
                            )}
                            {event.posteggio_numero && (
                              <span className="block">
                                Posteggio: {event.posteggio_numero}
                              </span>
                            )}
                            {event.saldo_trasferito_a && (
                              <span className="block text-blue-400">
                                Trasferito a: #{event.saldo_trasferito_a}
                              </span>
                            )}
                            {event.note && (
                              <span className="block italic">{event.note}</span>
                            )}
                          </td>
                        </tr>
                      ))}
            ...
            <CardContent className="pt-6">
              <div className="flex flex-wrap gap-4 items-end">
                ...
                <div className="w-[180px]">
                  ...
                  <Select
                    value={canoneFilters.mercato_id}
                    onValueChange={v =>
                ...
                <div className="w-[150px]">
                  ...
                  <Select
                    value={canoneFilters.tipo_operatore}
                    onValueChange={v =>
                      setCanoneFilters({ ...canoneFilters, tipo_operatore: v })
                    }
                  >
                    <SelectTrigger className="bg-[#0f172a] border-slate-700 text-white">
                      <SelectValue placeholder="Tutti" />
                    ...
                    <SelectContent className="bg-[#1e293b] border-slate-700">
                      ...
                      <SelectItem value="STRAORDINARIO">
                        Straordinari
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <div className="w-[150px]">
                  ...
                  <Select
                    value={canoneFilters.stato}
                    onValueChange={v =>
                      setCanoneFilters({ ...canoneFilters, stato: v })
                    }
                  >
                    <SelectTrigger className="bg-[#0f172a] border-slate-700 text-white">
                      <SelectValue placeholder="Tutti" />
              ...
              <div className="grid grid-cols-3 gap-4 mt-4">
                ...
                <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                  <div className="text-xs text-red-400 uppercase font-semibold">
                    Da Pagare
                  </div>
                  <div className="text-xl font-bold text-red-400">
                    ...
                    {canoneScadenze
                      ...
                      .reduce(
                        (sum: number, s: any) =>
                          sum + parseFloat(s.importo_dovuto || 0),
                        0
                      )
                  ...
                  <div className="text-xs text-slate-400">
                    {
                      canoneScadenze.filter(
                        (s: any) => s.stato_dinamico !== "PAGATO"
                      ).length
                    }{" "}
                    rate
                  </div>
                  ...
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                    ...
                    <div className="text-xl font-bold text-amber-400">
                      ...
                      {canoneScadenze
                        .reduce(
                          (sum: number, s: any) =>
                            sum + parseFloat(s.importo_mora || 0),
                          0
                        )
                        .toLocaleString("it-IT", {
                          minimumFractionDigits: 2,
                    ...
                    <div className="text-xs text-slate-400">
                      {
                        canoneScadenze.filter(
                          (s: any) =>
                            s.stato_dinamico === "IN_MORA" || s.pagato_in_mora
                        ).length
                      }{" "}
                      rate in mora
                    </div>
                  ...
                  <div className="text-xl font-bold text-green-400">
                    ...
                    {(() => {
                      ...
                      const totaleScadenzePagate = canoneScadenze
                        ...
                        .reduce(
                          (sum: number, s: any) =>
                            sum +
                            parseFloat(
                              s.importo_pagato || s.importo_dovuto || 0
                            ),
                          0
                        );
                      ...
                      if (
                        ...
                        const totaleRicariche = ricaricheSpunta.reduce(
                          (sum: number, r: any) =>
                            sum + parseFloat(r.amount || 0),
                          0
                        );
                        return (
                          totaleScadenzePagate + totaleRicariche
                        ).toLocaleString("it-IT", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        });
                      ...
                      return totaleScadenzePagate.toLocaleString("it-IT", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      });
                    })()}
            ...
            <CardContent>
              {isLoadingCanone ? (
                ...
                <div className="text-center py-8 text-slate-400">
                  ...
                  <p className="text-sm">
                    Usa "Genera Canone Annuo" per creare le scadenze
                  </p>
                </div>
              ) : (
                    ...
                    <tbody>
                      {canoneScadenze.map((s: any) => (
                        ...
                        >
                          ...
                          <td
                            className="px-4 py-3 text-slate-300 text-xs max-w-[150px] truncate"
                            title={
                              s.tipo === "STRAORDINARIO" && s.note
                            ...
                            {s.tipo === "STRAORDINARIO" ? (
                              <span className="text-amber-400">
                                ...
                                s.note.startsWith("{")
                                  ? JSON.parse(s.note).descrizione
                                  : s.note
                                      ?.replace("STRAORDINARIO: ", "")
                                      .split(" - ")[0] || "Straordinario"}
                              </span>
                            ) : (
                          ...
                          <td className="px-4 py-3 text-center">
                            {s.rata_totale > 1 ? (
                              <span className="inline-flex items-center gap-1">
                                <span
                                  className={`w-3 h-3 rounded-full ${(s.stato_dinamico || s.stato) === "PAGATO" ? "bg-green-500" : (s.stato_dinamico || s.stato) === "IN_MORA" ? "bg-red-500" : "bg-amber-500"}`}
                                ></span>
                                <span className="text-slate-300 text-sm">
                                  {s.rata_numero}/{s.rata_totale}
                                </span>
                              </span>
                            ) : (
                          ...
                          <td className="px-4 py-3">
                            {(s.giorni_ritardo_calc || s.giorni_ritardo || 0) >
                            0 ? (
                              <Badge
                                className={`${(s.giorni_ritardo_calc || s.giorni_ritardo) > 30 ? "bg-red-500/20 text-red-400" : "bg-amber-500/20 text-amber-400"}`}
                              >
                          ...
                          <td className="px-4 py-3 text-right font-bold text-white">
                            € {Number(s.importo_dovuto || 0).toFixed(2)}
                          </td>
                          <td className="px-4 py-3 text-right text-red-400">
                            {Number(s.importo_mora || 0) +
                              Number(s.importo_interessi || 0) >
                            0
                              ? `€ ${(Number(s.importo_mora || 0) + Number(s.importo_interessi || 0)).toFixed(2)}`
                              : (s.giorni_ritardo_calc ||
                                    s.giorni_ritardo ||
                                    0) > 0
                                ? `${s.giorni_ritardo_calc || s.giorni_ritardo} gg`
                                : "-"}
                          ...
                          <td className="px-4 py-3">
                            <Badge
                              className={`${
                                (s.stato_dinamico || s.stato) === "PAGATO"
                                  ? "bg-green-500/20 text-green-400"
                                  : (s.stato_dinamico || s.stato) === "IN_MORA"
              ...
              <CardContent>
                {isLoadingRicariche ? (
                  ...
                  <div className="text-center py-8 text-slate-400">
                    ...
                    <p className="text-sm">
                      Le ricariche dei wallet Spunta appariranno qui
                    </p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
          ...
          <Card className="bg-[#1e293b] border-slate-700">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">
                ...
                Lista Imprese{" "}
                {mercatiList.length > 0
                  ? `— ${mercatiList.find(m => m.id.toString() === selectedMercatoId)?.name || mercatiList[0]?.name || ""}`
                  : ""}
                <span className="text-sm font-normal text-slate-400 ml-2">
                  ({impreseConcessioni.length}{" "}
                  {impreseConcessioni.length === 1 ? "impresa" : "imprese"})
                </span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="flex flex-wrap gap-4 items-end mb-6">
                <div className="w-[150px]">
                  ...
                  <Select
                    value={impreseTipoOperatore}
                    onValueChange={v => setImpreseTipoOperatore(v)}
                  >
                    <SelectTrigger className="bg-[#0f172a] border-slate-700 text-white">
                      <SelectValue placeholder="Tutti" />
                    </SelectTrigger>
              ...
              {isLoadingImprese ? (
                ...
                <div className="text-center py-8 text-slate-400">
                  ...
                  <p>
                    {mercatiList.length === 0
                      ? "Caricamento mercato in corso..."
                      : "Nessuna impresa trovata per questo mercato"}
                  </p>
                </div>
              ) : (
                        ...
                        {impresa.tipo_record === "SPUNTA" ? (
                          ...
                          <span className="inline-flex items-center gap-2 px-2 py-1 text-xs font-medium rounded-md text-yellow-400 bg-yellow-400/10 border border-yellow-400/20">
                            ...
                            {impresa.wallet_balance !== undefined && (
                              <div
                                className={`flex items-center gap-1 pl-2 border-l border-yellow-400/20 ${impresa.wallet_balance > 0 ? "text-green-400" : "text-red-400"}`}
                              >
                                <div
                                  className={`w-2 h-2 rounded-full ${impresa.wallet_balance > 0 ? "bg-green-500" : "bg-red-500"}`}
                                />
                                <span className="font-bold">
                                  € {Number(impresa.wallet_balance).toFixed(2)}
                                </span>
                              </div>
                            )}
                          </span>
                      ...
                      {impresa.tipo_record !== "SPUNTA" && (
                        <div className="flex items-center justify-between text-sm">
                          <div className="text-slate-400">
                            Scadenza:{" "}
                            {impresa.valid_to
                              ? new Date(impresa.valid_to).toLocaleDateString(
                                  "it-IT"
                                )
                              : "N/A"}
                          </div>
                          <div className="flex items-center gap-1 flex-wrap">
                            ...
                            {impresa.scadenze_non_pagate -
                              (impresa.scadenze_in_mora || 0) >
                              0 && (
                          ...
                          {impresa.totale_dovuto > 0 && (
                            <span className="text-red-400 font-bold text-xs">
                              Dovuto: €{" "}
                              {Number(impresa.totale_dovuto).toFixed(2)}
                            </span>
                          )}
                        </div>
                      )}
            ...
            <CardContent className="pt-4">
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <Select
                  ...
                  onValueChange={value =>
                    setSanzioniFilters(prev => ({ ...prev, stato: value }))
                  }
                >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-red-400">
                      €{sanzioniTotali.non_pagato.toFixed(2)}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-400">
                      €{sanzioniTotali.pagato.toFixed(2)}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">
                      €{sanzioniTotali.totale.toFixed(2)}
                    </p>
                  </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Emesso:{" "}
                            {new Date(sanzione.issue_date).toLocaleDateString(
                              "it-IT"
                            )}{" "}
                            | Scadenza:{" "}
                            {new Date(sanzione.due_date).toLocaleDateString(
                              "it-IT"
                            )}
                            {sanzione.comune_verbale &&
                              ` | ${sanzione.comune_verbale}`}
                          </p>
                        </div>
                        <div className="text-right">
                          ...
                          <div className="flex gap-2 mt-2 justify-end">
                            <Button
                              ...
                              onClick={() =>
                                window.open(
                                  `${MIHUB_API_BASE_URL}/api/verbali/${sanzione.id}/pdf`,
                                  "_blank"
                                )
                              }
                            >
      ...
      {subTab === "notifiche" && (
        <div className="space-y-6">
          <NotificationManager
            ...
            mittenteId={
              getImpersonationParams().comuneId
                ? parseInt(getImpersonationParams().comuneId!)
                : 0
            }
      ...
      <Dialog
        open={showGeneraCanoneDialog}
        onOpenChange={setShowGeneraCanoneDialog}
      >
        <DialogContent className="bg-[#1e293b] border-slate-700 text-white sm:max-w-[400px]">
          ...
          <div className="py-4 space-y-4">
            ...
            <div
              className={`p-3 rounded-lg text-sm ${canoneFilters.mercato_id !== "all" ? "bg-blue-500/10 border border-blue-500/30" : "bg-amber-500/10 border border-amber-500/30"}`}
            >
              <div
                className={`font-medium mb-1 ${canoneFilters.mercato_id !== "all" ? "text-blue-400" : "text-amber-400"}`}
              >
                Mercato Target:
              </div>
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                ...
                {canoneNumeroRate === "2" &&
                  "Le rate saranno a 6 mesi di distanza"}
                {canoneNumeroRate === "3" &&
                  "Le rate saranno a 4 mesi di distanza"}
                {canoneNumeroRate === "4" &&
                  "Le rate saranno a 3 mesi di distanza"}
              </p>
            </div>
          </div>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShowGeneraCanoneDialog(false)}
              className="border-slate-600"
            >
              Annulla
            </Button>
            <Button
              onClick={handleGeneraCanoneAnnuo}
              className="bg-green-600 hover:bg-green-700"
            >
              Genera Scadenze
            </Button>
      ...
      <Dialog
        open={showPagamentoStraordinarioDialog}
        onOpenChange={setShowPagamentoStraordinarioDialog}
      >
        <DialogContent className="bg-[#1e293b] border-slate-700 text-white sm:max-w-[500px]">
            ...
            <div className="bg-slate-800/50 p-3 rounded-lg space-y-3">
              ...
              <div className="relative">
                <Label className="text-slate-400 text-xs">
                  Cerca Impresa (opzionale)
                </Label>
                <Input
                  value={straordinarioImpresaSearch}
                  onChange={e =>
                    handleSearchImpresaStraordinario(e.target.value)
                  }
                ...
                {straordinarioImpresaId && (
                  <div className="mt-1 text-xs text-green-400">
                    Impresa selezionata (ID: {straordinarioImpresaId})
                  </div>
                )}
              </div>
              ...
              <div>
                <Label className="text-slate-400 text-xs">
                  Seleziona Mercato {!straordinarioImpresaId && "*"}
                </Label>
                <select
                ...
                <div>
                  ...
                  {isLoadingPosteggi ? (
                    ...
                    >
                      ...
                      {straordinarioPosteggiList.posteggi_impresa.length >
                        0 && (
                        <optgroup
                          label="📍 Posteggi dell'impresa selezionata"
                          className="bg-green-900/30"
                        >
                          {straordinarioPosteggiList.posteggi_impresa.map(
                            (p: any) => (
                              <option
                                key={p.stall_id}
                                value={p.numero}
                      ...
                      {straordinarioPosteggiList.altri_posteggi.length > 0 && (
                        <optgroup label="📦 Altri posteggi del mercato">
                          {straordinarioPosteggiList.altri_posteggi.map(
                            (p: any) => (
                  ...
                  <div className="text-xs text-slate-500 mt-1">
                    Lascia vuoto per tutti i posteggi del mercato
                  </div>
                </div>
              )}
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleGeneraPagamentoStraordinario}
              className="bg-amber-600 hover:bg-amber-700"
            >
              Genera Avvisi
            </Button>
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              ...
              {selectedWallet?.type !== "SPUNTA" && (
                <>
                  {" - "}
                  {selectedWallet?.stall_number
                    ? `Posteggio ${selectedWallet.stall_number}`
                    : "Posteggio N/A"}
          ...
          <div className="py-6 space-y-6">
              ...
              isCalculating ? (
                ...
                <div className="bg-[#0f172a] p-4 rounded-lg border border-slate-700 space-y-3">
                  <div className="text-sm text-slate-400 mb-2">
                    Rate per {annualFeeData?.year || new Date().getFullYear()}:
                  </div>
                  {(() => {
                    // Trova la prima rata da pagare (NON_PAGATO o IN_MORA)
                    const rateNonPagate = walletScadenze.filter(
                      (r: any) => r.stato !== "PAGATO"
                    );
                    const primaRataDaPagare =
                      rateNonPagate.length > 0 ? rateNonPagate[0] : null;
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-slate-400 text-xs">
                              scad.{" "}
                              {new Date(rata.scadenza).toLocaleDateString(
                                "it-IT"
                              )}
                            </span>
                            {isPagatoInMora && (
                          ...
                          <div className="text-right">
                            {hasMora &&
                              (isPrimaRataDaPagare || isPagatoInMora) && (
                                <div className="text-xs text-red-400">
                                  +€ {rata.importo_mora.toFixed(2)} mora
                                </div>
                              )}
                            <span
                              className={`font-bold ${
                  ...
                  {(() => {
                    ...
                    if (rateNonPagate.length > 0 && primaRata) {
                      const importoTotale =
                        primaRata.importo_totale || primaRata.importo;
                      const hasMora = primaRata.importo_mora > 0;
                      return (
                        <>
                          {rateNonPagate.length > 1 && (
                            <div className="text-xs text-slate-500 mt-2">
                              * Paga prima la Rata {primaRata.rata} per
                              sbloccare le successive
                            </div>
                          )}
                          <div className="border-t border-slate-700 pt-2 flex justify-between font-bold text-lg">
                            ...
                            <span
                              className={
                                hasMora ? "text-red-400" : "text-blue-400"
                              }
                            >
                ...
                <div className="bg-[#0f172a] p-4 rounded-lg border border-slate-700 space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">
                      Tariffa {annualFeeData.year}:
                    </span>
                    <span>
                      € {annualFeeData.calculation.cost_per_sqm.toFixed(2)} / mq
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="border-t border-slate-700 pt-2 flex justify-between font-bold text-lg">
                    ...
                    <span className="text-blue-400">
                      € {annualFeeData.total_amount.toFixed(2)}
                    </span>
                  </div>
                  <div className="text-xs text-amber-400 mt-2">
                    * Nessuna scadenza generata. Genera le scadenze dalla tab
                    Canone.
                  </div>
                </div>
              ) : (
                <p className="text-red-400 text-center">
                  Impossibile calcolare il canone. Dati mancanti (Area, Tariffa
                  o Giorni).
                </p>
              )
            ...
            isCalculating ? (
              ...
              <div className="bg-[#0f172a] p-4 rounded-lg border border-slate-700 space-y-3">
                <div className="text-sm text-slate-400 mb-2">
                  Pagamenti Straordinari{" "}
                  {annualFeeData?.year || new Date().getFullYear()}:
                </div>
                {(() => {
                  // Trova la prima scadenza da pagare
                  const scadenzeNonPagate = walletScadenze.filter(
                    (r: any) => r.stato !== "PAGATO"
                  );
                  const primaScadenzaDaPagare =
                    scadenzeNonPagate.length > 0 ? scadenzeNonPagate[0] : null;
                  ...
                  return walletScadenze.map((scadenza: any, idx: number) => {
                    ...
                    const isPrimaScadenzaDaPagare =
                      primaScadenzaDaPagare &&
                      scadenza.id === primaScadenzaDaPagare.id;
                    const isBloccata = !isPagata && !isPrimaScadenzaDaPagare;

                    const importoDaMostrare =
                      scadenza.importo_totale || scadenza.importo;
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="flex flex-col">
                            <span
                              className={`text-sm font-medium ${isPagata ? "text-green-400" : isPrimaScadenzaDaPagare ? "text-white" : "text-slate-500"}`}
                            >
                              {causale}
                            </span>
                            <span className="text-xs text-slate-400">
                              scad.{" "}
                              {new Date(scadenza.scadenza).toLocaleDateString(
                                "it-IT"
                              )}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            {hasMora && isPrimaScadenzaDaPagare && (
                              <div className="text-xs text-red-400">
                                +€ {scadenza.importo_mora.toFixed(2)} mora
                              </div>
                            )}
                            <span
                              className={`font-bold ${
                          ...
                          {isPagata ? (
                            ...
                            <span className="text-xs bg-red-500 text-white px-2 py-1 rounded font-medium">
                              {scadenza.giorni_ritardo} gg MORA
                            </span>
                          ) : (
          ...
          <div className="py-4 space-y-3">
            <div className="bg-[#0f172a] p-4 rounded-lg border border-red-500/30">
              ...
              <div className="flex justify-between text-sm border-t border-slate-700 pt-2 mt-2">
                ...
                <span
                  className={`font-bold ${(walletToDelete?.balance || 0) >= 0 ? "text-green-400" : "text-red-400"}`}
                >
            ...
            <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-3">
              <p className="text-red-300 text-sm flex items-start gap-2">
                ...
                <span>
                  ...
                  definitivamente il wallet e tutte le transazioni associate.
      ...
      <Dialog
        open={showImpostazioniMoraDialog}
        onOpenChange={setShowImpostazioniMoraDialog}
      >
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-[#0f172a] rounded-lg border border-slate-700">
              <div>
                <Label className="text-white font-medium">
                  Calcolo Mora Abilitato
                </Label>
                <p className="text-xs text-slate-400">
                  Se abilitato, calcola automaticamente gli interessi di mora
                </p>
              </div>
              <button
                onClick={() =>
                  setImpostazioniMora({
                    ...impostazioniMora,
                    mora_abilitata: !impostazioniMora.mora_abilitata,
                  })
                }
                className={`w-12 h-6 rounded-full transition-colors ${impostazioniMora.mora_abilitata ? "bg-purple-600" : "bg-slate-600"}`}
              >
                <div
                  className={`w-5 h-5 bg-white rounded-full transition-transform ${impostazioniMora.mora_abilitata ? "translate-x-6" : "translate-x-0.5"}`}
                ></div>
              </button>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                step="0.001"
                value={(
                  impostazioniMora.tasso_interesse_giornaliero * 100
                ).toFixed(4)}
                onChange={e =>
                  setImpostazioniMora({
                    ...impostazioniMora,
                    tasso_interesse_giornaliero:
              ...
              />
              <p className="text-xs text-slate-500">
                Es: 0.0137% = tasso legale 5%/365 giorni
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e =>
                  setImpostazioniMora({
                    ...impostazioniMora,
                    tasso_mora_fisso: parseFloat(e.target.value) / 100 || 0,
                  })
                }
                className="bg-[#0f172a] border-slate-600 text-white"
                disabled={!impostazioniMora.mora_abilitata}
              />
              <p className="text-xs text-slate-500">
                Percentuale fissa applicata all'importo in mora
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e =>
                  setImpostazioniMora({
                    ...impostazioniMora,
                    giorni_grazia: parseInt(e.target.value) || 0,
                  })
                }
                className="bg-[#0f172a] border-slate-600 text-white"
              />
              <p className="text-xs text-slate-500">
                Giorni dopo la scadenza prima di applicare la mora
              </p>
            </div>
            ...
            {impostazioniMora.mora_abilitata && (
              <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-3">
                <p className="text-purple-300 text-sm">
                  <strong>Formula:</strong> Importo ×{" "}
                  {(impostazioniMora.tasso_mora_fisso * 100).toFixed(1)}% +
                  (Importo ×{" "}
                  {(impostazioniMora.tasso_interesse_giornaliero * 100).toFixed(
                    4
                  )}
                  % × giorni ritardo)
                </p>
      ...
      <Dialog
        open={showRegistraPagamentoDialog}
        onOpenChange={setShowRegistraPagamentoDialog}
      >
        <DialogContent className="bg-[#1e293b] border-slate-700 text-white sm:max-w-[450px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              Registra manualmente il pagamento di una sanzione
              (contanti/bonifico)
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedSanzione && (
            <div className="py-4 space-y-4">
              <div className="bg-slate-800 p-4 rounded-lg">
                <p className="font-medium text-white">
                  {selectedSanzione.verbale_code}
                </p>
                <p className="text-sm text-slate-400">
                  {selectedSanzione.impresa_nome}
                </p>
                <p className="text-xs text-slate-500">
                  {selectedSanzione.infraction_description ||
                    selectedSanzione.infraction_code}
                </p>
              </div>
              ...
              <div className="flex justify-between items-center p-3 bg-green-900/20 rounded-lg border border-green-500/30">
                ...
                <span className="text-2xl font-bold text-green-400">
                  €{parseFloat(selectedSanzione.amount).toFixed(2)}
                </span>
              </div>
            </div>
          )}
