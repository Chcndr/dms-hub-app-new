...
export default function WalletPanel() {
  ...
  const handleSaveImpostazioniMora = async () => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(`${API_URL}/api/canone-unico/impostazioni-mora`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        ...
        if (impostazioniMora.mora_abilitata) {
          await authenticatedFetch(`${API_URL}/api/canone-unico/aggiorna-mora`, { method: 'POST' });
          fetchCanoneScadenze();
        }
      } else {
  ...
  const handleGeneraCanoneAnnuo = async () => {
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(`${API_URL}/api/canone-unico/genera-canone-annuo`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDeleteScadenza = async (scadenzaId: number) => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(`${API_URL}/api/canone-unico/scadenza/${scadenzaId}`, {
        method: 'DELETE'
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleDeleteScadenzeAnno = async () => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(`${API_URL}/api/canone-unico/scadenze/${anno}`, {
        method: 'DELETE'
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleGeneraPagamentoStraordinario = async () => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const response = await authenticatedFetch(`${API_URL}/api/canone-unico/genera-pagamento-straordinario`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleOpenDeposit = async (wallet: WalletItem, companyName: string) => {
    ...
    try {
      ...
      if (scadenzeData.success && scadenzeData.semaforo && scadenzeData.semaforo.length > 0) {
        ...
        const res = await authenticatedFetch(`${API_URL}/api/wallets/calculate-annual-fee`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteWallet = async () => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const res = await authenticatedFetch(`${API_URL}/api/wallets/${walletToDelete.id}`, {
        method: 'DELETE',
      });
  ...
  const handleAzzeraWallet = async (walletId: number) => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const res = await authenticatedFetch(`${API_URL}/api/canone-unico/wallet/${walletId}/azzera`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
  ...
  const handleAzzeraTuttiWallet = async () => {
    ...
    try {
      const API_URL = WALLET_API_BASE;
      const res = await authenticatedFetch(`${API_URL}/api/canone-unico/wallets/azzera-tutti`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
