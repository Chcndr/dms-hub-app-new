...
export function HubMarketMapComponent({
  ...
  const marketBounds = React.useMemo(() => {
    ...
    const areaFeature = mapData.stalls_geojson.features.find(
      (f: any) =>
        f.properties?.kind === "area" && f.geometry?.type === "Polygon"
    );
    ...
    if (areaFeature && areaFeature.geometry.coordinates?.[0]) {
      ...
      allCoords = areaFeature.geometry.coordinates[0].map(
  ...
  const hubBounds = React.useMemo(() => {
    ...
    try {
      ...
      let areaGeojson =
        typeof selectedHub.area_geojson === "string"
      ...
      if (areaGeojson?.type !== "Polygon" || !areaGeojson.coordinates?.[0])
        return null;
        ...
        >
          ...
          {!routeConfig?.enabled &&
            ...
            (() => {
              const fixedCenter =
                marketCenterFixed ||
                (mapData.center
                  ? ([mapData.center.lat, mapData.center.lng] as [
                      number,
                      number,
                    ])
                  : null);
                    ...
                    >
                      ...
                      <div className="bg-[#ef4444] p-3 border-b border-red-800 flex justify-between items-center">
                        <div className="font-bold text-lg text-white flex items-center gap-2">
                          <span className="bg-white text-[#ef4444] rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold border border-red-800">
                            M
                          </span>
                          {market.name}
                        </div>
          ...
          {mode === "hub" &&
            ...
            allHubs.map(hub => {
              ...
              const fontSize =
                hub.livello === "capoluogo"
                  ? 11
                  : hub.livello === "provincia"
                    ? 10
                    : 9;
              ...
              const borderWidth = isProssimita
                ? 2
                : hub.livello === "capoluogo"
                  ? 2
                  : 1;
                  ...
                  <Popup className="hub-popup" minWidth={280}>
                    ...
                    >
                      ...
                      >
                        <div className="font-bold text-lg text-white flex items-center gap-2">
                          <span
                            className="bg-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold border border-purple-800"
                  ...
                  <Popup className="shop-popup" minWidth={280}>
                    ...
                    >
                      ...
                      >
                        <div className="font-bold text-lg text-white flex items-center gap-2">
                          <span
                            className="bg-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold"
                            style={{ color: "#9C27B0" }}
                          >
                            {shop.letter}
                          </span>
            ...
            (() => {
              try {
                ...
                if (areaData.type === "Polygon" && areaData.coordinates) {
                  const positions = areaData.coordinates[0].map(
                    (c: number[]) => [c[1], c[0]] as [number, number]
                  );
                  return (
                    <Polygon
          ...
          {mapData?.stalls_geojson?.features &&
            ...
            mapData.stalls_geojson.features
              .filter(
                f =>
                  (f.properties?.kind === "area" ||
                    f.properties?.type === "mercato") &&
          ...
          {mapData?.stalls_geojson?.features &&
            ...
            mapData.stalls_geojson.features.map((feature, idx) => {
              ...
              if (feature.geometry.type === "Polygon") {
                ...
                positions = coords[0].map(([lng, lat]: [number, number]) => [
                  lat,
                  lng,
                ]);
              ...
              const isSelected =
                String(selectedStallNumber) === String(props.number);
              ...
              const normalizeStall = (val: any) =>
                String(val || "")
                  .replace(/\s+/g, "")
                  .toUpperCase();
              const isTrulySelected =
                normalizeStall(selectedStallNumber) ===
                normalizeStall(props.number);
              const actualFillColor = isTrulySelected
                ? selectedColor
                : fillColor;
              // BORDO DELLO STESSO COLORE se selezionato, per mantenere dimensioni
              const actualBorderColor = isTrulySelected
                ? selectedColor
                : fillColor;

              return (
                <React.Fragment
                  key={`stall-${props.number}-${dbStall?.status || props.status}`}
                >
                  <Polygon
                    positions={positions}
                          ...
                          <div className="p-4 space-y-4">
                            ...
                            {(() => {
                              ...
                              const dimensionsSource =
                                dbStall?.dimensions || props.dimensions;
                              ...
                              if (dimensionsSource) {
                                ...
                                const normalized = dimensionsSource.replace(
                                  /,/g,
                                  "."
                                );
                              ...
                              if (isEstimated) {
                                const dims =
                                  calculatePolygonDimensions(positions);
                            ...
                            <button
                              ...
                              onClick={async () => {
                                if (!onConfirmAssignment) {
                                  alert(
                                    'Funzione "Conferma Assegnazione" non configurata!'
                                  );
                                  return;
                                }
                                ...
                                if (!dbStall?.id) {
                                  alert(
                                    "Impossibile trovare l'ID del posteggio!"
                                  );
                                  return;
                                }
                                ...
                                try {
                                  ...
                                  console.error(
                                    "[ERROR] Conferma assegnazione:",
                                    error
                                  );
                                  alert(
                                    "Errore durante la conferma assegnazione!"
                                  );
                                }
                              }}
                          ...
                          <div className="p-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              ...
                              <div className="bg-[#1e293b] p-2 rounded border border-gray-700">
                                ...
                                <div
                                  className="font-medium text-white truncate"
                                  title={`${positions[0]?.[0].toFixed(5)}, ${positions[0]?.[1].toFixed(5)}`}
                                >
                                  {positions[0]
                                    ? `${positions[0][0].toFixed(4)}, ${positions[0][1].toFixed(4)}`
                                    : "-"}
                                </div>
                              </div>
                            </div>
                            ...
                            {(() => {
                              ...
                              const dbWidth = dbStall?.width
                                ? parseFloat(dbStall.width)
                                : null;
                              const dbDepth = dbStall?.depth
                                ? parseFloat(dbStall.depth)
                                : null;
                              ...
                              if (!hasDimensions && positions.length > 0) {
                                try {
                                  ...
                                  console.warn(
                                    "Errore calcolo dimensioni geometriche:",
                                    e
                                  );
                                }
                              }
