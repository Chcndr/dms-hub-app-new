...
export function HubMapComponent({
  ...
  const mapCenter: [number, number] =
    ...
    (showItalyView
      ...
      : hubData
        ? [
            parseFloat(hubData.center_lat || hubData.lat),
            parseFloat(hubData.center_lng || hubData.lng),
          ]
        : [43.5, 12.5]);
    ...
    <>
      ...
      <div style={{ height, width: "100%" }}>
        ...
        >
          ...
          {showItalyView &&
            allHubs
              .filter(hub => isValidCoord(hub.lat, hub.lng))
              .map(hub => (
                <Marker
                    ...
                    >
                      ...
                      <div className="bg-[#9C27B0] p-3 border-b border-purple-800 flex justify-between items-center">
                        <div className="font-bold text-lg text-white flex items-center gap-2">
                          <span className="bg-white text-[#9C27B0] rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold">
                            H
                          </span>
                          {hub.name}
                        </div>
          ...
          {!showItalyView && hubData && (
            <>
              ...
              {hubData.area_geojson && hubData.area_geojson.coordinates && (
                <Polygon
                  positions={hubData.area_geojson.coordinates[0].map(
                    ...
                    <div className="text-sm">
                      ...
                      {hubData.area_sqm && (
                        <div className="text-gray-600">
                          Superficie: {hubData.area_sqm} m²
                        </div>
                      )}
                      <div className="text-gray-600 mt-1">
                        Negozi: {hubData.shops?.length || 0}
                      </div>
                    </div>
                  </Popup>
                </Polygon>
              ...
              {hubData.shops &&
                hubData.shops
                  .filter(shop => isValidCoord(shop.lat, shop.lng))
                  .map(shop => {
                      ...
                      >
                        <Popup className="shop-popup" minWidth={250}>
                          ...
                          >
                            ...
                            >
                              <span
                                className="bg-white rounded-full w-8 h-8 flex items-center justify-center text-lg font-bold"
                            ...
                            <div className="p-3 space-y-2 text-sm">
                              ...
                              <div className="bg-[#1e293b] p-2 rounded border border-gray-700 mt-2">
                                ...
                                <div className="font-mono text-xs text-gray-300 flex justify-between">
                                  <span>
                                    Lat: {parseFloat(shop.lat).toFixed(6)}
                                  </span>
                                  <span>
                                    Lng: {parseFloat(shop.lng).toFixed(6)}
                                  </span>
                                </div>
                              </div>
                            </div>
