...
export default function LoginModal({
  ...
  const handleSPIDLogin = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Errore durante il login SPID"
      );
      setLoading(false);
    }
  };
  ...
  const renderEmailForm = () => (
    <div className="space-y-4">
      ...
      <input
        ...
        onKeyDown={e =>
          e.key === "Enter" && !isRegistering && handleEmailLogin()
        }
        className="w-full px-4 py-3 bg-card/80 border border-border rounded-xl text-white placeholder:text-muted-foreground focus:outline-none focus:border-primary"
      />
      ...
      {isRegistering && (
        <label className="flex items-start gap-2 text-xs text-muted-foreground cursor-pointer">
          ...
          <span>
            ...
            e il trattamento dei dati personali ai sensi del GDPR (Reg. UE
            2016/679)
          </span>
        </label>
      )}
