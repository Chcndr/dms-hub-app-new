...
### 2026-05-09 — Piano correzioni UI Anagrafica: formazione, adempimenti, attestati e associazione

Intervento chirurgico richiesto sulla pagina impresa `AnagraficaPage.tsx` per correggere quattro aree emerse da verifica mobile. Nel tab **Formazione**, le card dei corsi disponibili e delle iscrizioni devono mostrare titolo, categoria, descrizione, durata, sede, date, prezzo e stato corso/iscrizione, evitando righe anonime composte solo da sede e prezzo. Nel tab **Qualifiche**, le schede di adempimento mancante devono avere una informativa apribile con dettaglio dell’obbligo, normativa e azione richiesta, oltre alla segnalazione già esistente verso l’associazione. Nel tab **I Miei Attestati**, quando è disponibile un PDF (`documento_url`, `attestato_pdf_url` o `attestato_pdf_id`) deve comparire l’icona occhio per aprire l’attestato, coerentemente con il pannello Team/Formazione. Nella sezione **La Mia Associazione**, i pulsanti **Contatta** e **Richiedi Servizio** devono diventare azioni effettive: il primo deve aprire i contatti della scheda pubblica dell’associazione, il secondo deve caricare i servizi dell’associazione e permettere l’invio di una richiesta tramite endpoint esistente.
